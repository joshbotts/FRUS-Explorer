...
- **Build collections** — curated, ordered, sectioned sets of documents with your own connecting prose and derived apparatus enrichments — and export them as PDF, HTML, or Word files. This is how you assemble a source packet for a seminar paper or a course reader for your students.
- **Organize by project**, so the notes and collections for your dissertation chapter stay separate from the ones for the courses you're teaching.
...
| Option | Description |
|--------|-------------|
| **Entire Corpus** | All 553 published volumes — about **3.3 GB** of XML to download, plus another **9–10 GB** of search index once everything is indexed; downloading takes time depending on your connection |
...
The panel sits over a **word cloud drawn from the series itself**, counted from the FRUS text at build time and shipped inside the app, cycling between its **Concepts** and **Topics** vocabularies. Pick a subseries and the cloud becomes that era's vocabulary before you commit. Each row states its rough size — the whole series about 3.3 GB, a single volume a few megabytes — and the subseries picker shows how many volumes each era holds; exact per-volume sizes appear later in **Settings → Volumes & Storage**. Click **Continue**, or **Skip** to download nothing now. If you are offline, the picker shows the bundled catalog only and nothing is queued; add volumes from Settings once you are back online.

**Step 3 — Ready.** A confirmation — *You're all set* — telling you that volumes are downloading, or, if you skipped, that nothing is downloading yet. If you have no project, the app creates one named **My Research** and makes it active; projects organize notes, tags, and collections around one research effort (Section 10), and you can rename it or add others later. Click **Finish** — the main window opens and downloading and indexing continue in the background.

> **How much should you download?** The choice isn't binding — volumes add and remove freely later. A reasonable start: the **subseries covering your research period**, adding neighboring eras as your chronology firms up. If you have **13–14 GB** of storage to spare, downloading the whole corpus is the most powerful configuration — corpus-wide search and analytics sharpen with every volume you index. And a fair amount of the app works before you download anything: the About the Series dashboards, the archival collection authority, and the volume-level Top Subjects all ship inside the app.
...
2. **Move through the volume.** **⌥⌘↓** advances to the next document in reading order, **⌥⌘↑** goes back — or click the faint chevrons at the document's left and right margins, which brighten as you hover.

3. **Highlight a passage.** Select a sentence. A dark pill — the **floating selection bar** — appears above the selection. Click one of its four **color dots** and the passage is highlighted in that color, persistently and across your devices.
...
5. **Open the Research rail.** Press **⌘⇧R**. The rail slides in beside the document with seven tiles — **Cite**, **Word Cloud**, **Sources**, **Graph**, **Related**, **On the Map**, **Share** — above expandable **Topics**, **Summary**, **Notes**, **Tags**, and **Collections** sections. This is the per-document research surface; everything you do *about* a document starts here.
...
9. **Save what worked.** Click the **bookmark** button (*Save this search*) beside the search field to save the query with all its filters; the neighboring **Saved searches** button lists and re-runs them.
...
The Mac app is built around one **main window** (the document reader) surrounded by specialized tool windows you summon as needed. If you have the screen for it, a productive arrangement is the main window centered with Search on one side and your Project Home or an analytics window on the other.

### 4.1 The Toolbar

**Center — document title.** When a document is open, a compact `volumeId/documentId` title (e.g., `frus1969-76v01/d42`) appears here, updating as you navigate; hover it for the document's heading. With no document open the title reads *FRUS Explorer*, and on a narrow window the title drops out before the tools do.

**Trailing edge — five controls.** The two ▾ entries are menus grouping the specialized windows:

| Control | Shortcut | Opens |
|---------|----------|-------|
| **Search** | ⌥⌘F | The full-text Search window |
...
| **My Research ▾** | — | **Research** (⌘⌥R — every annotated document), **Collections** (⇧⌘K), **Archives Visits** (Section 14.8), and **Complete History…** (Section 16.2) |
| **Research rail** | ⌘⇧R | Show or hide the per-document Research rail (4.2); dimmed until a document is open |

![The macOS main-window toolbar — the centered volumeId/documentId title and the trailing Search, Browse, Analytics, My Research and Research-panel controls.](screenshots/macos/toolbar.png)

### 4.1a The Menu Bar

Five of the app's menus are its own, and most windows and commands in this manual have a menu home as well as a toolbar one:

| Menu | What it holds |
|------|---------------|
| **File** | **New Window** (⌘N), **Open Document in New Window**, **Print…** (⌘P) |
| **Document** | **Previous Document** (⌥⌘↑), **Next Document** (⌥⌘↓), **Add Note…** (⌘⇧N), **Highlight Selection ▸** (the four colors; Yellow on ⌘⇧H), **Show Research Panel** (⌘⇧R) |
| **Find** | **Find in Document…** (⌘F), **Find Next** (⌘G), **Find Previous** (⌘⇧G), **Search…** (⌥⌘F), **Search Tips…**, **Corpus Browser…** (⇧⌘B), **Citation Lookup…** (⇧⌘F) |
| **Collection** | The open collection's commands (Section 12) |
| **Analytics** | The same seven windows as the toolbar's Analytics ▾ menu (Section 15) |
| **Research** | **Project Home** (⌘⇧P), **New Project…**, **Switch Project ▸**, **Research** (⌘⌥R), **Collections** (⌘⇧K), **Archives Visits**, and **History ▸** — **Documents Visited**, **Searches Executed**, **Complete History…** |
| **Window** | Besides the standard items, cold doors to the **Cross-Reference Graph** and **Source Explorer** windows |
| **Help** | **FRUS Research Guide** (Section 16.3) |

### 4.2 The Research Rail

The **Research rail** (⌘⇧R) is the per-document research surface, a trailing panel beside the document. On a wide window it sits side-by-side with the text; below roughly 900 points of window width it becomes an overlay that slides in over the document. It opens with a **RESEARCH** header, then a three-column **tile grid** of seven tools:
...
| **Related** | The Related Documents window — a ranked list of the documents most related to this one (Section 8.4) |
| **On the Map** | The Semantic Map window centered on this document, among the documents whose language is most like it (Section 15.6) |
| **Share** | Zotero send, Zotero-file export, and citation sharing (Section 11) |

Below the tiles sit expandable accordions — **Topics** (the document's detected subjects, most distinctive first, shown only when there are any; each opens the Topic Index, Section 6.4), **Summary** (AI summaries, Section 13), **Notes**, **Tags**, and **Collections** (Sections 9 and 12). The Topics caption says what it is: detected automatically from the text, not editorial subject headings, so some are wrong.

The rail header's **ⓘ** button opens a **Document tools** popover describing each of the seven tiles — and, at the bottom of that popover, carries the document's **Classification**: whether the app treats what you're reading as a *document* or an *editorial note*, with the control to correct it. FRUS's own tagging is occasionally wrong, and because the app trusts it, the mistake reaches the type badge, search's document-type filter, counts, and exports. **Reclassify as Document…** / **Reclassify as Editorial Note…** records your correction: the body restyles immediately, every filter and badge follows it on all your devices, and the popover then shows what FRUS tags it as so the disagreement stays visible. Both track FRUS's own later corrections: if the Office of the Historian fixes the same tagging, the popover says so — *FRUS now tags this the same way, so your correction no longer changes anything* — and **Restore FRUS's Classification** puts the new value back, not the reading recorded on the day you disagreed. Fully reversible — **Restore FRUS's Classification** in the same popover, or manage every correction (with per-row Undo) under **Settings → Search → Classification Corrections…**. One caveat the confirmation dialog also states: the bundled series-analytics dashboards are computed from the published corpus and cannot see your corrections.

**Highlighting is not a rail button.** To highlight, **select text in the document body**: the **floating selection bar** (a dark pill) appears at the selection with four color dots — click one to save the highlight — plus **Excerpt** (freeze the passage into a collection, Section 12.3), **Look Up** (hand the text to Source Explorer, Section 14.2), and **Note** actions. For a selection inside a footnote, the color dots and Excerpt are disabled; Look Up and Note remain available. A list's own heading and the numbers the volume printed beside its items — **SUBJECT**, **PARTICIPANTS:**, **(1)**, **a.** — are drawn on the page but are not part of the text a highlight is anchored to: a selection that starts on one begins at the item's first word, one that ends on one stops just before that item, and a highlight or excerpt of numbered paragraphs keeps their words without the numbers. The same bar, with the same behavior, appears on iPad and iPhone.

![The Research panel beside the Long Telegram — the tile grid and the Topics, Summary, Notes, Tags and Collections accordions, here holding a note and a "Containment" tag.](screenshots/macos/research-strip.png)

### 4.3 The Document View

The central area displays the open document ("Select a document to begin" when none is). As you navigate — search results, cross-references, the browser — the window builds a navigation stack; the toolbar's back button steps back through it. **⌥⌘↑** / **⌥⌘↓** (or the margin chevrons) move through the volume in reading order. **⌘F** opens a compact find bar pinned to the top of the document; **⌘G** and **⌘⇧G** step through the matches, and **Done** closes it.

![A rendered FRUS document filling the main window — heading, dateline, body with linked names and footnote markers, and the footnotes section.](screenshots/macos/document.png)
...
- **Indexed count** — a green dot and *N volumes indexed* at the left edge. While indexing runs, a **Learn** button beside the progress opens the FRUS Research Guide (Section 16.3).
- **iCloud sync** — one chip at the right edge. With nothing wrong it reads **iCloud Sync**, **Syncing…**, or **Synced**. Otherwise it names the most fundamental problem, and only that one: **Local Only** (orange — iCloud could not start; hover for the diagnostic), then **Not Signed In** (orange — sign in via System Settings → Apple ID to sync), then **Zone Missing** (red — the private sync zone is absent; relaunch, or use **Settings → Data & Recovery → Fix iCloud Sync**), then **Sync Error** — click that one for detail and an **Open Sync Diagnostics** button. A Mac that is not signed in shows **Not Signed In** alone, not the sync errors that follow from it.
...
On the first launch after install, and on a launch where iCloud is still pulling your research down, the word cloud fills the otherwise-empty window behind the wordmark — clearing when the wait actually ends, with indexing progress always taking the screen ahead of it.
...
| People | The Corpus Browser's **People** door (Section 6.3) |
| Topics | The **Browse all topics** link in Search's facets, or a scope bar's topic chip (Section 6.4) |
| Archives Visits | My Research ▾ menu, or **Research ▸ Archives Visits** (Section 14.8) |
| Collections | ⇧⌘K |
| Research | ⌘⌥R |
| Citation Lookup | ⇧⌘F |
| Corpus / Person / Cross-Reference / Archival / Semantic Analytics, Chronology, Word Cloud | Analytics ▾ menu |
| History (Complete History) | My Research ▾ menu, or **Research ▸ History ▸ Complete History…** |
| Project Home | ⌘⇧P (disabled when no project is active) |
| FRUS Research Guide | Help menu |
| Settings | ⌘, |

**Picking up where you left off.** A main window with no document open offers **Continue reading** with the last document you had open — one click to return, nothing opens by itself. The offer comes from your reading history, so it survives reinstalls and follows you from your iPad or iPhone; a document whose volume you have since removed is skipped rather than offered.
...
A volume not yet on this Mac carries a hollow download glyph in its row in the Corpus Browser. Click the row and press **Download Volume** on its page, which states the approximate size; the status bar counts what is queued, and **Settings → Volumes & Storage** shows each volume's live progress. Downloaded volumes index automatically on completion. For bulk additions, the **Download from GitHub…** button at the top of **Settings → Volumes & Storage** opens the full browse list at three grains — **Entire Corpus**, **One or More Subseries**, or **Individual Volumes** — and states the XML total with its roughly 2.8× index overhead before you commit; **Sideload XML File…** beside it imports a volume file you obtained separately. And anywhere you reach a document in an undownloaded volume — a cross-reference, a citation-lookup result, a graph node — the app offers the download rather than dead-ending.
...
- Its row in Browse carries a **Side-loaded** badge in place of the publication badge. (On
  iPhone and iPad the subseries header also counts side-loaded volumes separately; the Mac
  sidebar counts them in with the subseries total.)
...
**Settings → Volumes & Storage** is the single destination (Section 17.2 walks the whole pane). The essentials: a **Storage used** bar split into **XML**, **Index**, **Summaries**, and **Vectors** (the semantic-vector files, Section 5.3a); **Free Up Space…**, which lists only volumes with nothing of yours attached and that the app can download again (a side-loaded volume is never offered), ordered by how long since you opened each — never-opened first — with an estimated recovery figure for whatever you tick, and asks before removing; **Keeping Current → Check for Corrections**, which compares your copies against the published ones (the Office of the Historian does occasionally correct volumes — worth running before you cite something contentious), with **Update** / **Update All** preserving your notes, highlights, tags, and summaries, and **Refresh Available List** re-reading the published catalog. After an update, an **After an Update** section lists the volumes whose changed documents carry your research — split into changed text, changed apparatus, and documents no longer in the volume — with **Open Research** to review them under *Changed by an update* and **Mark Reviewed** to clear a volume once you have. Under **Advanced**, **Concurrent downloads** sets how many volumes fetch at once (1, 2, 3, 4, or 6; the default is 4). Your annotations are never touched by any storage or index operation — removing a volume removes the text, not your work.
...
![The Corpus Browser — the sidebar's Browse, Your sets and Subseries sections, and a volume's page with its Top subjects, front matter and contents.](screenshots/macos/browser.png)

### 6.1 The Sidebar and Navigation Levels

The sidebar has two sections. **Browse** holds the cross-cutting doors: **All Volumes** — one catalog of every volume, searchable, with a control arranging it by **Title** (A–Z on each volume's distinctive title segment, since almost every full title begins with the same series boilerplate; early annuals file under **#**), **Published** (print year, newest first, in decade groups — the year from the title page, not a declassification date), **Era**, or **Length** (document count, largest first); **Administrations** — every presidency from Lincoln on with its reach, drilling to the volumes whose documents *cover* the term (dated to it, never by publication; a volume spanning two administrations appears under both, and post-corpus presidencies stay visible but dimmed); **Editors** — the volume editors by surname, each drilling to their volumes in publication order (spelling variants merge into one entry; the printed forms and citations are never altered); and **Archives** — the series by its printed source notes, in three side-by-side lenses: ten *provenance-type* doors (Central Decimal File, presidential libraries, lot files…) drilling to volume lists with per-volume shares; the Source Explorer's own *collection* index, grouped by **repository**, National Archives **record group**, or not at all, whose records now push their full detail in this window — citing-volume rows stay in your stack instead of jumping the sidebar; and the central-file *classes*, divided by the filing schedule in force, since the same number means different things under different schedules. The last two have a **Sort** menu — **document count** or name (a class number in the file's own order, so *711.11* before *711.2*), **ascending or descending**, remembered on this Mac, with every group and era collapsible from its header or all at once with **Collapse All** / **Expand All** — and collections whose citations name no record group, nearly every presidential-library collection among them, are listed together at the end; and **Clusters** — the corpus's 171 computed language groups, the same grouping the semantic map (Section 15.6) draws as regions, listed largest first with each cluster's four-term sampled label, document count, and era histogram. A cluster drills to its documents grouped by volume — paged, since the largest holds nearly 38,000 — with the usual coverage line and Download/Index rows, plus **See on the semantic map** (opens the map zoomed to the cluster, region card ready) and **Save as Working Corpus** (capped at 7,500, truncation stated). The screen states its own limits: labels are sampled terms, not subject headings; about 28% of the corpus (89,449 documents) belongs to no cluster and cannot be reached here; era bars reflect volume coverage, not document dates. A **Your sets** section holds **Working Corpora** — your captured document sets, each opening as one always-rendering list grouped by volume, with a coverage line up top ("N of M documents indexed on this device"), real headings for indexed volumes, and gray identifier rows with **Download**/**Index** buttons for volumes this Mac doesn't hold (capture stays in Search results and the map's lasso; there is nothing to create here) — and **My Scopes** — your custom volume scopes, openable and now *editable and creatable* right here: a pushed editor renames a scope, removes members, and adds volumes through the catalog in picker mode; right-clicking any volume row anywhere in the browser offers *Add to Scope…* and *New Scope from Volume…*; and every volume list's toolbar has **Save as Scope…** to capture the whole slice (an administration's volumes, an editor's volumes) with the name pre-filled. **My Scopes** also has a **New Scope** button for an empty scope you fill from the catalog. **Browse Within This Scope**, which narrows the Subseries hierarchy to one scope's volumes, is on iPhone and iPad only: the Mac browser has no such filter, so here you open a scope to see its volumes. **Subseries** below it is the classic era list, and every volume list carries a **Search volumes…** field matching on title or volume number.

![The Corpus Browser with Browse ▸ All Volumes selected — the catalog in Published order, each volume with its print year, document count and download status.](screenshots/macos/browse-all-volumes.png)

- **Subseries view** — the volumes a subseries contains: title, volume number, publication date, **document count**, and whether each is downloaded and indexed (the same row the iPhone and iPad lists use, so the two browsers can no longer drift apart). Each row also carries two glyph buttons — a **Word Cloud** of that volume and its **Cross-Reference Graph** — and right-clicking a row offers **Add to Scope…**, **New Scope from Volume…**, and **Word Cloud**; a subseries row's context menu offers a word cloud of the whole subseries.
- **Volume view** — front matter, chapters, and appendices, each chapter with its document count. A **Top subjects** section lists the subjects most characteristic of the volume — automatically detected topics (not editorial subject headings, so an occasional mistag is possible), grouped by category, and shown even before the volume is downloaded, so you can size up an unfamiliar volume before committing to it. Click a subject to see every other FRUS volume covering it across the corpus and jump straight to any of them; the sheet also offers **Archival profile of these volumes**, opening Archival Analytics on the collections those volumes draw on (Section 15.5). Once indexed, the volume page adds a summary strip — persons in its List of Persons, cross-references, how many documents are dated, editorial notes, and the date span its documents cover; while indexing runs, the page reports the stage (*Reading…*, *Storing batch N of M…*, *Finalizing index…*).
- **Chapter view** — individual document listings with dateline, source note, and document number.

### 6.2 Filtering the Browser

Two toolbar buttons narrow the sidebar: an arrow that flips the subseries list between **newest first** and **oldest first**, and a download glyph that toggles **Show downloaded only**. Every volume list carries a **Search volumes…** field that matches on title or volume number. (Filtering by subject tag is an iPhone and iPad feature; the Mac browser reaches subjects through **Top subjects** on the volume page and the **Topics** window, Section 6.4.)
...
The same person often appears across many volumes under different name forms ("Kissinger, Henry A.", "Kissinger, Henry", "Kissinger, Henry A. Laurence"). FRUS Explorer consolidates these into **one identity**, so you aren't chasing one person through a dozen entries. Each row shows the canonical name; a subtitle with the **role** a volume's List of Persons gives and the **active years** (*Secretary of State · 1973–1977*); and a **mention count** — the number of distinct documents referencing this identity across your indexed corpus. The active years run from the earliest to the latest year the volumes themselves give for the person: the years in their Lists of Persons, read by the words around them (*until January 3, 1979* is when someone left, *from June 13, 1982* when they began, and a post held *until his death on November 22, 1963* ends that year), and the dates of the indexed documents that mention them. They are never a preface or other front matter, which is dated when the volume was prepared; never a year a List of Persons gives only as a death (*died 1896*); and never the birth and death years of the name-authority data, which the detail sheet shows as life years instead. A search field filters by name.
...
Click a person for their detail panel. A **reconciled-identity seal** there marks a person matched to the bundled name-authority data, and a reconciled person's **life years** sit under the name, beside the role the Office of the Historian's people register gives them — the register's own years, with a year it lacks filled from the Principal Officers and Chiefs of Mission register. Those say when the person lived; the **Active** row below them is the volumes' span, as in the list.

![The person detail sheet for "Kissinger, Henry A.", scrolled to its VIAF and Wikidata links and its Career section — Secretary of State, 1973–1977, from the Department's Principal Officers and Chiefs of Mission register.](screenshots/macos/people-detail.png)
...
- Where the app is uncertain whether two identities are the same person, it flags the row **Possible duplicate**, and the detail sheet's **Possibly the Same Person** section offers **Merge**. You can also merge two people yourself — **Merge with another person…** in the detail panel, or right-click a person's row — for the cases the deliberately cautious automatic grouping keeps apart; a confirmation names both people, and warns when they look like genuinely different people (each matched to a distinct authority entry).
...
- Reconciled identities carry **View on VIAF** and **View on Wikidata** links where those identifiers exist — roughly a quarter to a third of the authority entries, so their absence is not a sign the match failed. A person who appears only in a volume's front-matter list, with no indexed document mentions, says so in place of **Find all mentions**.
...
![The Topics window narrowed to the Cold War area (its chip reads "Topic area: Cold War — 6 topics"), with Berlin crisis open: its documents and volumes in the series and on this Mac, and its covering volumes.](screenshots/macos/topic-index.png)
...
Press **⌥⌘F** (or choose **Find ▸ Search…**) for the Search window — the caret lands in the query field even if the window was already open behind something else, so you can start typing immediately. Search is full-text across every volume you've indexed, ranked by relevance (BM25) with English stemming, so *negotiate* also matches *negotiated* and *negotiating*. For a researcher the important properties are less the ranking than the accountability: the app shows you the exact query it ran, describes the whole result set rather than just the visible page, and — while **Log Research Sessions** is on (Section 17.4) — records every search as evidence of your method.

![The Search window — "Berlin blockade" in Keywords mode, the ranked results with highlighted snippets, citations and dates, and the Facets panel open on its Subjects section.](screenshots/macos/search.png)

### 7.1 Running a Search

Type and press Return. The search itself runs on Return — at this corpus size it must — but the Query Inspector strip (Section 7.3) rewrites itself as you type, and any filter you change re-runs the search immediately. Three **Search in** chips under the field choose what is searched: **Documents** (full text, headings, datelines, and source notes), **Notes** (your research notes), and **Summaries** (generated summaries) — so your own annotation layer is searchable alongside the primary sources. Tags narrow a search (**My Tags**, Section 7.4) rather than being searched as text. Beside the field sit **Tips** (Section 7.2) and **Find by Citation** (⇧⌘F, Section 11.4).
...
- **Reading the count.** "N of M results" is the plain case. "total unavailable" means the engine could not price the whole match; "· narrowed from N" appears while Facets is open and a facet filter is in force; inside a working corpus the line reads "N in this corpus". A ⚠ advisory appears when the match ran past the cap: *Showing N of M matches — narrow with a date range, volume filter, or more specific terms.*
...
![The Search window with the Tips panel open under the results.](screenshots/macos/search-tips.png)

| Syntax | Example | Effect |
|--------|---------|--------|
| Plain keywords | `berlin crisis` | Both words required (AND implied) |
| Quoted phrase | `"cold war"` | The words together, in that order |
| Boolean AND / OR / NOT | `kissinger AND détente` · `vietnam NOT laos` | Explicit boolean logic, in any case; quote an operator (`"and"`) to search for the word itself. Words side by side bind tighter than `OR`: `cold war OR peace` finds *cold* with *war*, or *peace* |
| Exclusion | `-laos vietnam` · `vietnam AND NOT laos` | Excludes the word from the terms it is typed with, wherever it sits among them; `-word` and `NOT word` are the same, except before `NEAR(…)`, which only `NOT NEAR(…)` excludes |
| Grouping | `(aqaba OR tiran) AND navig*` | Parentheses nest up to 32 deep — a query nested deeper is not run |
...
| Prefix wildcard | `negoti*` | negotiate, negotiated, negotiating… (suffix wildcards like `*tion` are not supported). The prefix is matched against word stems, so keep it short — `negotiat*` misses *negotiations*, which `negoti*` finds |
| Proximity | `NEAR("military guarantee" Europe, 30)` | Both operands within 30 words, in either order |
| Exact word | `=containment` | The word with stemming turned off — not *contain*, *containing*, *container*; capitalization still does not matter |

**Quotation marks.** Straight (`"cold war"`), curly (`“cold war”`, `„cold war“`) and guillemet (`«cold war»`) quotation marks all make a phrase, so a phrase typed with smart quotes on, or pasted from a document, is searched as the phrase it is. A phrase cannot hold quotation marks of its own: every mark opens or closes a phrase, so `"the “iron curtain” speech"` searches its four words separately. Leave the inner marks out — `"the iron curtain speech"` still matches a document that prints them.

**Exclusions and OR.** A `-` or `NOT` in front of a word applies to the terms it is typed with and does not reach across `OR`: `cold -korea OR war` finds documents with *cold* but not *korea*, plus every document with *war*. To exclude from every alternative, group them: `(cold OR war) -korea`. `AND NOT korea` means the same as `-korea`, and doubling the mark (`NOT -korea`) still just excludes *korea*. An `OR` alternative made only of exclusions has nothing to search for on its own, because full-text search finds documents by the words they contain: in `cold OR -korea` the `-korea` is left out, the search runs as `cold` alone, and the Query Inspector marks *korea* **NOT APPLIED**. Put in parentheses beside a word to search for, the same `OR` is searched exactly and nothing is left out — `war (cold OR -korea)` finds the *war* documents that mention *cold* or do not mention *korea*. A phrase or word prefix restored with a saved search gives it the same footing: beside one, `cold OR -korea` — or `-korea` on its own — is searched exactly against that phrase or prefix. Excluding a group that holds a word to search for works differently, because it reverses the marks inside the group: each word the group searches for is excluded, and each word excluded inside it becomes a word to search for. That is how an exclusion can combine the alternatives of an `OR` typed inside the group — `NOT (cold OR -korea)` finds the *korea* documents without *cold*. So `cold OR -(war -korea)` means *cold*, or *korea*, or no *war*: it runs as `cold OR korea`, and only *war* — now a part that only excludes — is marked **NOT APPLIED**. A group made only of exclusions still just excludes them: `cold -(-korea)` is `cold -korea`.

**Proximity (`NEAR`).** Two words merely co-occurring in a document tells you little — a hundred-page volume can mention almost anything twice. `NEAR` asks whether the ideas appeared *together*. Operands may be words, phrases, or prefixes (`NEAR(militar* europ*, 20)`); the distance defaults to 10; `NEAR` works in any case, and `NEAR/20(a b)` is accepted as an alternative spelling. `OR`, `NOT` and parentheses cannot go *inside* a `NEAR` — write `NEAR(a b, 20) OR NEAR(c d, 20)` instead, and check the Query Inspector (Section 7.3) for how a query was read. To exclude a proximity, write `NOT NEAR(…)`: a `-` in front of `NEAR` does not exclude it.

**A search needs a word to find.** A query made only of exclusions — `-korea`, or `NOT korea` — has nothing to search for, and neither does one whose parentheses nest more than 32 deep. The Query Inspector says so as you type (Section 7.3), and if you search anyway, the window says the query has nothing it can search for and points you to the tips. A search that fails for another reason — every **Search in** chip turned off, or a meaning search that cannot run — shows its own message there too. A `NEAR(…)` holding a boolean, a minus sign or parentheses is refused the same way, and the message quotes the NEAR back to you: dropping the NEAR would run a different search — the distance would be looked for as a word, and a minus sign inside would exclude that word from the whole corpus.

**Exact word (`=`).** Stemming is usually what you want, but a search for *containment* is really a search for *contain* — and on a corpus this size that can turn a handful of genuine hits into a five-figure count of *containers* and *containing*. Prefix a term with `=` for the literal word: it works per term (so `=containment polic*` mixes literal and stemmed), and the result **count** narrows too (so a figure you cite is the figure you matched). It applies only where every match must contain the word you marked: where the marked word is required, or where every `OR` alternative marks it, as in `=cold war OR =cold peace`. Once a word's `=` applies, it applies wherever you search for that word with an `=`, so `(=cold OR war) =cold` finds only the literal *cold*. A marked word is the same word however you capitalize it, whatever punctuation you type at either end of it, and with or without a single accent such as *é*, *ü* or *ç*, so every alternative of `=Cold war OR =cold. peace` marks *cold*, and both marks apply. Letters such as *ø*, *ł* and *đ* are different letters rather than accented ones, so `=Gomułka` and `=Gomulka` mark two words. Where a match need not contain the marked word — when only one `OR` alternative marks it, or on a word an excluded group leaves optional — the `=` is ignored and the query still runs, stemmed there, so `=cold OR war` still finds documents with *war* and no *cold*. Only a marked word makes a mark apply, so an unmarked copy of the same word, which matches by its stem, does not: `(=cold OR war) cold` still finds a document with *colds* and *war*, and `=cold war OR cold peace` one with *colds* and *peace*. Excluding a group can also make a mark apply, as `NOT (war OR -=cold)` finds the literal *cold* without *war*. The `=` is always ignored on a prefix, inside `NEAR(…)`, and on a word the index splits into several terms, such as `=U.S.S.R.`.

### 7.3 The Query Inspector

Under the search controls, a strip shows the **FTS5 expression your query actually became** — the string sent to the database, not a paraphrase. Expand it to see, per term: the **index form** it was reduced to, with a warning when that is broader than what you typed (`containment` is searched as `contain`); how many documents contain the term **across everything you have indexed** (free to compute, updates as you type); and, on request, the **exact count within your current filters** (a real query per term, so it sits behind a button). The gap between the two counts is itself information — a term common in the corpus but rare in your scope is telling you something about your scope. Excluded terms are tagged **EXCLUDED** and get no count of their own. A term tagged **EXACT** is filtered to the literal word; a word whose `=` the search ignores (Section 7.2) gets no tag and is counted by its stem, as it was searched. Where a word's `=` applies, every term that searches for it with an `=` is tagged, however it is spelled, so in `(=cold OR war) =cold` and `(=Cold OR war) =cold.` both *cold*s are tagged, and in `(=cold OR war) cold` neither is. A term tagged **NOT APPLIED** was typed but is not part of the expression: it sat in a part of the query that only excludes — an `OR` alternative made only of exclusions, or a word inside an excluded group, like *war* in `-(war -korea)` — and a search needs something to find, so that part was left out (see *Exclusions and OR* in Section 7.2). When anything was left out, a line under the expression says the search is **narrower than typed** — the only sign when what was left out is an operator word rather than a term. When a query cannot run at all — nothing is left to search for once its exclusions apply, as in `-korea` alone, or its parentheses nest more than 32 deep — the strip says so where the expression would be. Text with nothing to search for in it yet, such as a lone `(` or `"` while you type, shows no strip. A term tagged **ADVANCED** came from the phrase, prefix or excluded terms a restored saved search carries beside what you typed, and is counted like any other. The expression line never hides; if you publish method appendices, it is the line you copy.

### 7.4 Filters

Active filters sit in a row of **tokens** under the Search in chips. Each token names its field and value, opens its editor when clicked, and has an ✕; **+ Filter** adds one (date, years, volume, person, tags, type, front matter, or subject); **Advanced…** opens the full **Filters** popover, which applies every change immediately while the results stay visible. With nothing set the row reads *no filters — all types, front matter included, everything indexed*, and when a working corpus, project scope, phrase, excluded term, or prefix is in force it says *also narrowed by … — see Advanced* instead.

| Filter | Description |
|--------|-------------|
| **Volume / Subseries scope** | One or more volumes, or a whole era. When a volume finishes indexing, the summary card in the Corpus Browser offers **Search this volume**, which opens Search scoped to it |
...
| **My Tags** | Documents you have tagged. The list is read from your tags each time you open **Advanced…**, in the order you set in Settings ▸ Tags. Each tag carries a count of how many of the current results hold it, recomputed when a search runs: after a keyword search that is every document the search matched, and after a search by meaning it is how many of the closest matches carry the tag |
| **Person** | One person, chosen by name — the same reconciled identities the People Browser lists (Section 6.3). The token is cleared, with an alert, if a re-index drops the identity it named |
| **Project** | When a project is active: **Entire corpus**, **History** (documents this project has opened), or **Focus**, plus **Only new to this project** |
| **Search scope** | The three Search in chips as toggles — **Include document text**, **Include summaries**, **Include research notes** — plus **Include front matter** |
| **Type** | A token with its own menu — **Both**, **Primary documents**, or **Editorial notes** — added from **+ Filter** rather than from the popover |
| **Front matter** | A token — **Included** or **Excluded** (preface, introduction, persons lists, sources) |
| **Result Preview** | Snippet length (1–10 lines) for this window |

A **Topic** token chosen in the Topics window can be removed here but not edited (*Chosen in Topics. Remove it here, or pick another there.*), and a **Subject** token opens the facet panel. The same volume/subseries scope is shared with Corpus Analytics, so a query can be charted and read against the identical corpus subset. The open-ended groups (Working Corpora, Volume Scopes, Detected Topics, My Tags) start collapsed — but a group always opens itself when a warning stands.

### 7.5 Facets

Click **Facets** in the window's titlebar for a panel that describes your **whole result set** rather than one row at a time — by year, volume, person, document type, archival provenance, and subject. Sections compute when you open them, not when you search, so the panel never slows a query you weren't going to inspect.
...
Each section carries a **Sort** (Most documents, A–Z, Z–A; Oldest or Newest first for Years) and a **Show** (Top 5, 10, 25, or Show all). Years, document type, provenance, and subjects open showing everything; Volumes and People open at the top 25 — a common-term search can span hundreds of volumes and thousands of people — with a **Filter N rows** field, above a hundred rows, that ignores accents and capitals (`agustsson` finds Ágústsson). Captions state the shape of what you see — *the top three hold N% of the match*, *N matched documents carry no date* — and the Subjects footer says where its rows come from: detected topics from the Office of the Historian, matched by name, not the volumes' own markup. In Meaning mode the panel stays available and describes the closest matches (*Counted over the results themselves, not the whole corpus*). Alphabetical sorting puts most people in last-name order, since FRUS records names as "Last, First."

### 7.6 Four Ways to Read a Result Set

The titlebar carries a four-way **Reading** picker — exactly one is on — with **Save as Working Corpus…**, **Checklist**, and **Facets** beside it; the sort bar below keeps only the count, **Show**, and **Sort**:

- **List** — the ordinary ranked list.
- **Timeline** — the matches placed by year, with a **Chart | List** switch and a note of how many undated documents were left out. A caption above the chart reminds you that the shape is a relevance ranking's, not the whole match's.
- **Concordance** — every occurrence of your term lined up on the term itself, so a screen of hits reads as usage rather than as a list (the KWIC view, if you've used corpus-linguistics tools).
...
They do not all count the same thing, and each panel names the set it used (the Reading picker's tooltip says it too): the concordance shows **this page**; the timeline and collocates cover the results retained for this search; **Facets** reads the whole match. When you are about to quote a number, that distinction *is* the number.

### 7.7 Checklist Review Mode

Working systematically through a few hundred matches is a triage task, and **Checklist Mode** turns the list into a shrinking to-do list. Click the labeled **Checklist** button in the titlebar; with it on, a result disappears from the list — and from the timeline — as soon as you **open it** by any route, or when you right-click it and choose **Mark Reviewed**. A subtle "N reviewed hidden" banner counts what you've cleared; **All Results Reviewed** appears when nothing is left; clicking the button again brings everything back.
...
Reading picker (Timeline, Concordance, Collocates) disabled and the "Search in" scope chips hidden, while **Facets** stays available and describes the closest matches;
...
| **Semantically similar (experimental)** | Language that reads alike, whether or not the words match. On by default at half weight — below the archival and citation signals, so it shapes the list without dominating it — and its slider moves it either way |
...
Each row shows the document's header, volume, and dateline, plus **"why related" icon chips** naming the contributing signals, strongest first — the same icons the weights panel uses. A chip states only what its signal can honestly report: **cited 3×** for cross-references; for archival provenance, the container and its size — **Lot 54 D 270 · 1 of 1,063** — because sharing a six-document lot file is a finding and sharing one of seven thousand is a filing-cabinet coincidence. (Those two *find* candidates rather than scoring them, so no percentage; date, corpus proximity, and shared people report percentages, which for them are real measures. The two experimental axes name the distinctive words the pair actually shares — **shares: Kearsarge, blockade** — and fall back to a percentage only when no such overlap can be put into words.) A shared-topics chip names the topics themselves — **topics: Berlin blockade, Economic sanctions** — most distinctive first, because two documents sharing *Berlin blockade* is a finding and two sharing *War* is not.

Clicking a row opens the document in the main window while the Related Documents window **stays open beside it** — a work list you step through — and open windows restore across relaunch with their tuning intact. A **Scope** picker narrows the pool (This volume / This subseries / All volumes — the default, since cross-corpus reach is the point); **Adjust weights** exposes a slider per signal, re-ranking on release, with your tuning persisted as the seed for every Related window you open afterward and **Reset to app defaults** returning every slider (enabled once you have moved one). The tuning records only the sliders you actually move, so a signal you leave alone follows the app's default even after that default changes. **Shared topics** and **Semantically similar** are both live at half weight, below the editorial signals on purpose — detected topics are matched by name and variants, recall-oriented, and a few are wrong; the semantic axis ships without the quality gate its design called for, so your verdicts are its test: right-click a semantic row for **Semantic match: helpful** / **not useful** (the verdicts collect under Settings → Data & Recovery → Semantic Match Feedback, Section 17.5). An empty list says why — nothing indexed shares the anchor's provenance or citations, or the scope is too narrow; switch to **All volumes**. Footers are honest: one counts the qualifying documents beyond the list; another appears when the candidate pool itself was cut on a very large archival container — *"Ranked from the first 120 of 1,063 documents that share this anchor's archival container"* — and narrowing the scope is what reaches the rest.

Below the ranked list, **Beyond your library** counts the documents in volumes you have *not* downloaded that read as close to this one as the matches above — the semantic tier ships for the whole corpus, so it can see past your library. Each volume row states how many matching documents it holds and offers **Download Volume**; the count reads *at least* when the scan stopped early. For the same question asked visually, the rail's **On the Map** tile places this document on the semantic map with its closest neighbors (Section 15.6).
...
![The cross-reference graph — the focus document and its references along a date axis, teal nodes for the archival material its footnotes cite, and the reference list panel.](screenshots/macos/cross-reference-graph.png)

- **Reading it.** Nodes are documents positioned left-to-right by date; arrows point from the citing document to the cited one; larger nodes are more connected; line weight and labels convey link strength; color encodes direction — **blue** nodes cite the focus, **orange** nodes are cited by it, **gray** nodes are two or three hops out, and **teal** nodes are unprinted archival citations (below). A legend and info popover explain every channel, so meaning never depends on color alone; a reset-viewport button re-frames; animations respect Reduce Motion.
...
- **Depth.** Show 1, 2, or 3 hops from the focus; a document with very few direct references auto-expands to 2 hops and says so.
- **Layouts.** **Timeline** places each document at its date along a time axis — cited documents to the left, citing ones to the right, undated ones in an **Undated** column; drag across the axis to zoom to a date range, and the clear button restores the full span. **Network** is a spring layout arranged by connections alone. Timeline is unavailable when too few documents carry dates.
- **Groups.** Dense neighborhoods collapse into date groups and clusters labeled with their document count; right-click for **Expand Date Group** / **Expand Cluster**. Recentering pushes onto a graph-local history, and **Back to previous focus** returns to the earlier document.
- **Node actions.** Hover the middle of a line to read the footnote text that made the reference; click a node to see its details in the panel; right-click for *Recenter Graph*, *Open in Main Window*, and *Archival Neighbors…*; scroll to zoom, drag the background to pan. Nodes in undownloaded volumes are drawn distinctly, and clicking one offers the download. Opened from **Window ▸ Cross-Reference Graph** rather than from a document, the window starts at a volume picker: **Volume Connections** draws every other volume this one cites or is cited by, corpus-wide, and **Browse Documents** lets you pick a document and open its own graph.
- **Inbound citations are complete, whatever you have downloaded.** The app ships the corpus-wide list of every cross-volume citation — 8,637 of them, into 5,747 documents from 185 volumes — so a document cited by six others across the series shows six inbound arrows even if you hold only two of those volumes. Citing documents from undownloaded volumes appear as nodes without titles, with a banner counting them; download a volume and its nodes fill in.
...
Notes file under the active project (Section 10). On the Mac, a note written under a different project does not appear in the rail until you switch to that project (**Research ▸ Switch Project**); the Research window's **All Notes** list, filtered by project, is where you see every note across projects. In collection exports, notes render as clearly separated **"Research Note"** blocks after the document body: your voice, kept typographically distinct from the document's own footnotes.
...
Many researchers assign meanings to the colors — evidence for chapter 2 in yellow, historiographic leads in green — and the Research window's **By Highlight** grouping then works as a filing system. To attach a note to a passage you just highlighted, use the **Add Note to Highlight** button that appears in the rail's Notes accordion right after the highlight is created (the selection bar's **Note** action creates a document-level note instead).

**Stale highlights.** If the underlying TEI source of a document has been updated since you highlighted it, a banner appears at the top of the document saying whether the text itself changed or only the notes, source note, or heading, and the affected highlights show in amber. The banner's **Review…** button opens a sheet listing every highlight on the document with its standing, and every quotation frozen from it: **Confirm** keeps a highlight where it is and clears the warning (on every device you are signed in to), **Remove…** deletes it after confirmation, and — when the app can find the exact words you highlighted somewhere else in the corrected text, in one place only — **Move Here** relocates the highlight after showing you the passage in its new surroundings. Below the highlights, every **quotation** you froze from the document is checked against the text as it now reads, the same check that runs on the way out of a collection (12.10): each row says what the app could establish — that the words are still there, that they are not, or why it could not tell — and adds where the quotation was taken from when that is worth saying. Quotations are read-only in this sheet — an excerpt belongs to its collection, and the collection editor is where you change one. **Summarize Again** makes a fresh summary of the corrected text using the same prompt as the newest one, and keeps the old: they stack up in the document's Summary block, newest first. It appears only when the document already has a summary, when Apple Intelligence is available on the Mac, and when the document still exists — an update that removed the document leaves nothing to summarize. If the prompt that made the newest summary has since been deleted, a line above the button names the one that will be used instead. The sheet does not tell you *which* summaries predate the correction, because it cannot tell reliably: it would have to compare each summary against a record of the text it was made from, and that record moves for reasons that are not corrections — one Rebuild Index rewrites it across your whole library, it is kept unchanged on a document an update removed, and it is not kept at all for summaries made before this version. Your **notes** are listed by their first line and open in the note editor from here, **Edit Tags…** opens the tag picker, and if the document is seeded into an archive-visit plan there is a row per plan that opens it in the **Archives Visits** window — so an annotation the sheet has just named is one click away rather than a trip back through the document. The plan opens on its own terms: the editor takes a whole plan and does not scroll to the document that sent you there. **Mark Reviewed** clears the document from the Research window's *Changed by an update* list — and, with iCloud sync, on your other devices too, a few seconds after each next syncs or when it next opens. The same sheet is available from *Review Changes…* on a Research row, which is also the only route for a document an update removed from its volume. The app never moves a highlight on its own.

**The Research window** (⌘⌥R) gathers every annotated document in one place — highlights appear as colored strip excerpts beneath each document header, showing the verbatim highlighted text, with a sidebar grouping by collection, tag, and highlight color. Three sidebar rows sit beneath *All Research Documents*: **Contains Notes**, which narrows to the documents you have actually written on rather than merely marked; **All Notes**, which lists the notes themselves with filters for project, tag, and free text; and **Changed by an update**, the documents a volume update altered under your research, which the Review sheet's **Mark Reviewed** clears (Section 17.2). When a project is active, a **Project Home** row sits above them. That list used to be **Settings → Notes**; the pane is gone, because it held a list of your work and not a single setting.

**Writing a note.** The body takes formatted text — a formatting bar sits above the field, and the **Formatting** switch in the section header turns it off if you prefer plain text (a per-Mac preference that changes nothing already stored). Whatever you write is also kept as plain text, which is what search, the exports and your Zotero library read. **Tags** and **Projects** are pickers rather than long lists of switches: the row shows what is applied, and opening it gives you a searchable list you can drag to reorder — put the tags and projects you are working with now at the top. It is the same order as **Settings → Research → Tags** and **Projects**, so setting it in either place sets it in both. That order follows you to your other devices, and it rearranges the alphabetical list rather than replacing it — anything you have not placed stays in alphabetical order behind the ones you have. Note formatting is kept on your devices and in iCloud, but a `.fruscollection` file carries its notes as plain text, so formatting does not survive sharing a collection and reimporting it.
...
**Settings → Research → Projects → New Project…** (or **Research ▸ New Project…**) asks for a name and an optional research question. Everything else that steers a project — **Focus tags**, **Focus subjects**, attached **Collections**, and the suggestion weighting — is set on Project Home (10.4). A date range set during first-run onboarding is shown on Project Home as *From … Through …* and pre-fills the Search date filter; it cannot be edited afterwards.


### 10.3 Switching and Merging

Switch the active project from the menu bar (**Research ▸ Switch Project**) or the **Active Project** picker in Settings → Research → Projects; the change is instant, and all new annotations from that point belong to the new project. The Corpus Browser and Search windows show the active project's research question in a "Working on:" banner — a label, not a control. Both follow the order of the project list in Settings — right-click a project for **Move to Top**, **Move Up** and **Move Down**; the note editor's project list follows it too. To fold one project into another, right-click it in Settings and choose **Merge into Another Project…**: all notes, summaries, collections, and history entries unify under the destination, and the source project is deleted.

### 10.4 Project Home and Leads

**Project Home** (**⇧⌘P**, or **Research ▸ Project Home**; disabled when no project is active) is the active project's dashboard, in its own window. It opens with four counts — **Collections**, **Notes**, **Documents Visited**, **Searches Run** — and the research question, editable in place. **Focus tags** and **Focus subjects** choose which of your tagged documents and which volumes anchor the suggestions; **Collections** attaches collections to the project (**Manage**; a collection can belong to several projects) and holds **Plan a Visit** (Section 14.8); **Recent Activity** lists what you read, wrote, and searched under the project. Its **Suggested Next** section suggests documents related to the ones you have already gathered under the project, ranked by the same signals as Related Documents (Section 8.4). Each lead shows the document's header, how many of your project's documents it relates to, and a few lines of what it actually says — its summary if it has one, otherwise its opening text — so you can judge it without opening it. Leads are the closest thing the app has to a research assistant: gather ten documents under a project and it will tell you what else belongs in the pile.
...
Beside the Collections section header sits **Plan a Visit** — create-or-open for the project's **Archives Visit** (Section 14.8), which opens in the **Archives Visits** window, brought forward on that plan. The button is available once the project holds a document — through an attached collection, a note or a focus tag — or already has a plan, and it follows **Manage** at once: attach a collection there and it enables without reopening Project Home. A new plan seeds from the project's engaged documents (the same set that seeds the leads engine) with the research question as the inquiry's topic sentence. Once the project has a plan, the button opens its most recently changed one — which may be a plan you created from a collection, Source Explorer or the Archives Visits list while the project was active, since those belong to it too, or one that came with a project you merged into this one. **Re-seed from Project** in the plan's menu pulls in new engaged documents and offers the project's current research question as the topic sentence, on request — never automatically; it asks before replacing a topic that says something else.
...
The window has no permanent sidebar; you switch collections from the **collection picker** at the left of the toolbar — a pop-up menu listing every collection with its document count, plus **New Collection…** (⌥⌘N), **Import Collection…**, and **Manage Collections…** (a sheet where a collection's name is an inline field; right-click a collection there for **Duplicate** — a fully independent copy including composition, sections, and prose — **Word Cloud**, or **Create Static Snapshot**). The middle column is the **Contents** outline; the **live preview** sits beside it. The rest of the toolbar carries the authoring verbs:

| Control | What it does |
|---------|--------------|
| **＋ Add** | Insert content: **Add Documents…** (⇧⌘A), **Add Section Heading**, **Add Note Block**, **Add Passages…** (highlight excerpts; the Collection menu calls it **Add Highlighted Passages…**), an **Apparatus ▸** submenu of the five generated blocks (**Insert Apparatus** in the Collection menu), and **Add to Archives Visit…** — seeds a plan from this collection's documents (14.8) |
...
**Which collections the window lists.** Every collection across all your projects, by default; with a project active, a banner offers **Scope to "\<project\>"**, and **Show All** brings the rest back (a per-session choice). A collection you create, import, or snapshot while a project is active is attached to that project automatically.

![The Collections window — the toolbar collection picker, the Contents outline, the live preview, and the document inspector.](screenshots/macos/collections.png)
...
Selections append in the order you picked them; repeats are allowed — the picker marks them **In collection**, and the duplicate row in Contents carries an **Also in collection** chip; finishing confirms with "Added N documents."
...
- **Prose blocks** (note blocks) are your connecting commentary, written in a rich-text editor — bold, italic, underline, color, and hyperlinks (with **Remove Link**) from the visible formatting bar, and the standard **Format** menu's **⌘B**, **⌘I**, **⌘U** in any prose block or the introduction (links become real hyperlinks in HTML and Word, and print as visible URLs in PDF). This is where a course reader's head-of-section framing lives. A long block shows its opening lines in the Contents outline — at most six — ending in an ellipsis; click it to edit the whole block, and it goes back to its opening lines when you click another row or field. The introduction in the ⚙ Collection popover works the same way. **Undo** (⌘Z) and **Redo** (⇧⌘Z) reach a block's last edit even after you have clicked into another block, and what the undo leaves is what the collection keeps — in exports, on your other devices, and when you open it again. In the introduction, an undo or a redo made while the ⚙ Collection popover is open is kept the same way.
- **Excerpts** are frozen verbatim quotations, rendered in every export as styled block quotes with automatic source citations (and the source highlight's color as an accent bar). Because the excerpt stores the exact passage, it renders even when the source volume isn't downloaded. Three ways to create one: **Add Passages…** (pick from your highlights on the collection's documents, several at once); select a passage while reading and choose **Excerpt** on the floating selection bar; or **Insert as Excerpt** on any highlight row in a document's inspector. The quoted text itself is never edited — it stays exactly as the source printed it on the day you captured it, which is what makes it checkable later if the volume is corrected.
...
**Front matter.** The ⚙ Collection popover's **Name** section holds the collection name, a **Subtitle (title page)**, and an author line (its placeholder is your active project's name — used only if you type it). Its **Title Page & Introduction** section holds an **introduction** written in the same rich-text editor and rendered as the opening prose after the table of contents, an optional **colophon** — a closing line noting the collection was compiled with FRUS Explorer, with its document and volume counts — and **Stamp active project on export**, which prints the active project's name on the export. All optional. (A further option there, **Append the query log**, attaches your project's method appendix to the export — off by default; see Section 17.5 before switching it on.)

### 12.4 Sorting by Date

The **Sort** menu re-orders documents chronologically in one of two modes: **Across the Whole Collection** (one continuous chronology — documents may move past your headings) or **Within Each Section** (documents sort only inside their heading's section, so your sectioning survives). Both leave headings, prose, and excerpts where you placed them. The menu-bar **Collection ▸ Sort by Date** item always sorts across the whole collection; choose **Within Each Section** from the toolbar **Sort** menu. Right-click any row for **Move Up** / **Move Down** when dragging is awkward, and a collapsed heading shows how many rows it hides.

### 12.5 Document Rows and the Inspector

Each document row is a scannable report — title, volume, date, and small labeled chips — **N notes** / **Notes off**, **Highlights off**, **Headnote**, **See also** — that appear only when a value differs from the collection default, so an untouched row stays clean. The row's controls: a **⚙ Configure** pill (opens the inspector), a **book icon** that opens the document in the app's own reader (with your notes, highlights, and cross-references), an arrow that opens the published text on history.state.gov, and remove.
...
- **Title override** — replaces the derived title in the table of contents and the export heading for this document only; leave blank to keep the derived one.
- **Body depth** — Default / Full / Summary only / Index for this one document.
- **Provenance** and **Excerpts in This Collection** — the archival source note with its cross-reference count, and this document's inserted excerpts. Select a *heading* row instead and the same panel shows **Section Details** with the section's defaults.
...
| Setting | Options |
|---------|---------|
| **Default body depth** | *Full document*, *Summary only* (requires Apple Intelligence; generates on demand), or *Index / outline* (citation, date, notes — no body) |
...
| **Include word cloud** | Prepend a frequency overview to the PDF, HTML, and Word exports |
...
|--------|----------------|
| **Teaching reader** | Full document text with source notes and your research notes, footnotes off, a header-and-dateline table of contents, and a **Persons Index** and **Chronology** appended — built for close reading in a classroom |
| **Briefing packet** | **AI summaries** in place of full text, **Key takeaway** headnotes switched on, and a concept word-cloud overview — a fast, skimmable read |
| **Source dossier** | A compact index/outline body with each document's archival source note, footnotes and highlights off, a citation-style table of contents, and a **Sources & Archives** block appended — an archival finding aid |
...
Exports always include the collection title and a linked table of contents, and each file is named after the collection. A collection with no name exports as **Untitled Collection** — the file's name, and the title of a PDF, HTML or Word export. The file is written to a temporary location first; the **Export Complete** sheet offers **Reveal in Finder** and **Save To…** (Return) to copy it where you want it.
...
FRUS Explorer integrates with Apple Intelligence to generate document summaries **on-device** — no document content is sent to any server. Summaries are stored locally, sync through iCloud, and are indexed for search — the newest summary of each document, so an older one in the ‹ › history is not searchable. In Search, the **Summaries** chip includes or excludes that text. Treat them as what they are: a reading aid and a triage tool, never a substitute for the document, and the app labels them accordingly wherever they appear in something you export.
...
Open the Research rail (⌘⇧R), expand **Summary**, and click **Summarize this Document**. The first summary uses a standard prompt; **Change prompt** picks another and regenerates the moment you click it. Thereafter the block names the prompt that made the summary you are reading, and **Regenerate** runs *that* prompt again — so regenerating a summary you wrote a custom prompt for gives you another in the same voice, not a standard one. If you have since deleted that prompt, the block says which prompt Regenerate will substitute before you press it. Regenerating keeps the old summary: when a document has more than one, ‹ › chevrons and an *n*/*N* counter step through them, newest first. The block's header reads **AI SUMMARY · ‹prompt name›**, naming the prompt only while it still exists on this Mac; a failure shows as an orange banner in the block. After a volume update, the Review Changes sheet (Section 9.2) offers **Summarize Again**, which makes one more summary of the corrected text with the newest summary's prompt and stacks it here. A summary carries who wrote it — the model, the model then edited by you (a headnote you corrected, Section 12.5), or you — and exports label it accordingly.
...
**Your own prompts** are created in **Settings → Research → Summarization**: name it, choose **General** (free text) or **Structured** (define fields by name and type), write the instructions, save. The seven structured standards are offered as templates; **Start from Scratch** gives a plain prose prompt. The standard prompts are the app's — they cannot be edited, and an update that rewords one rewords it on every device — so start from one with **Use as Template** or duplicate it to make it yours; your own prompts have **Edit**, **Duplicate**, and **Delete**. The editor's formats are **General (prose)** and **Structured (fields)**, with **Add Field** for a field name and optional description, and every prompt must contain `{{DOCUMENT}}` where the text goes. The pane shows Apple Intelligence's status, a **Generated so far** count, and per-prompt use counts.

### 13.3 Long Documents

Documents too long for a single model call are chunked at paragraph boundaries, summarized independently, and combined — hierarchically for very long documents, partial summaries themselves reduced in stages — so even a million-character treaty text completes rather than failing. The Mac shows no indicator when this has occurred — the summary reads the same either way; whether a summary was made in sections is recorded on it (the JSON export carries it), and during a batch run the progress line names the part being processed (13.4).

### 13.4 Background Summarization

To summarize many documents at once: **Settings → Research → Summarization → New Batch Run…**. Choose a scope — **Volume**, **Subseries**, **User Tag**, **Saved Search** (its current results), **Date Range** (earliest and latest year), or **My Volume Scopes** (this one shows how many of the scope's volumes are downloaded) — set the concurrency limit (1 to 6, default 2), and click **Start** (it becomes **Stop** while a run is in progress). When Start is unavailable, the line beneath it says why — no prompt yet, no volumes downloaded, a scope with none of its volumes downloaded, an incomplete date range. Apple Intelligence generates one summary at a time internally, so a higher concurrency doesn't make the model faster and makes the first summary take longer to appear — it helps when your Mac is busy with other work. Progress shows in Settings and in the main window's status bar; closing the sheet does not stop a run, and the rest of the app stays fully responsive.
...
|-----------|-----------|:---:|
| **State Dept. decimal files (1910–1963)** | The NARA finding-aid page for the file's filing period, plus the relevant **filing manual** PDF where applicable. The period is the file's own: a number written with a date (`611.93/12–854`, 8 December 1954) is filed in that year's period whatever the document's date, unless its month and day are the document's own under another year — a misprinted year — and a number without a date is placed by the document's year | No |
...
| **Pre-1910 Central Files** | A **bundled index**: 1906–1910 Numerical File citations link to the digitized microfilm roll (e.g. M862), and pre-1906 records resolve within the country-arranged diplomatic series across the full pre-1906 range, 19th-century datelines included (a chapter of correspondence with a foreign legation in Washington, such as *British legation.*, resolves to that legation's Notes to or from Foreign Missions, never to an instruction or despatch) — plus the chronological runs — Consular Instructions, Notes to and from Foreign Consuls, Domestic Letters, Letters Received, and the Special Agents series — matched by the document's date alone. For a pre-1906 document that prints a **serial** above its text (*No. 428*), the digitized-records section repeats it as a second handle for the rolls — they are browsed by eye, and the serial sits on the images beside the date — labelled **Instruction No.** for the Department's instruction to a post, **Despatch No.** for a post's despatch, and a plain **No.** for a note or for a Department letter the register cannot settle. It is not a NARA identifier (about one pre-1906 document in ten carries one). Where the Office of the Historian's register of chiefs of mission shows a Department letter's addressee as the U.S. chief of mission at that post on that date, **Diplomatic Instructions** comes first, marked **Likely**, with a line naming the minister, and the Notes to Foreign Missions reel is not offered; where it cannot (no one of that name at the post on that date, allowing 90 days after a tenure; two people; a foreign title on either side; a descriptive addressee) both reels stay, marked **Possible**. The dateline and chapter are read from the search index, so the answer is the same from the Browser, Research, History or a collection — and when the check cannot run, the section says why rather than showing nothing | No |
...
| **CIA records** | The panel states that CIA accession records are not described in the NARA Catalog, so no query is offered (the CREST link is on iPhone and iPad only; on the Mac, search CREST yourself with the job number the panel parsed) | No |
...
**Scans before the trip.** Where NARA has digitized the file itself, a **Digitized Scans** box lists the scanned ranges covering your document's decimal file number, with the image count and a PDF link for each. When NARA digitized the file in overlapping sets, every covering range is listed narrowest first — the widest is not wrong, the narrowest is simply most specific, and the extra candidates are marked **Possible**. Where the decimal class is scanned but your file number falls outside every range, the box says so rather than sending you into the wrong several-hundred-page reel.

**What NARA states about a resolved series.** Beneath the catalog link on a resolved lot file, the box repeats NARA's own facts about the series: its **NARA Creator** (with **See all series by this creator**), **Access** and **Use** status with any restrictions spelled out, **Series Dates**, **Extent**, the **Held At** reference unit, and any **Finding Aids** — the answers to the questions a reference request would otherwise cost you an email. And what you copy carries its hedge: an exported or copied resolution keeps its caveat, so an unverified keyword match leaves the window still saying it was one.
...
Select any text in a document body — a lot number, a decimal identifier, an archival keyword — and choose **Look Up** on the floating selection bar: the Source Explorer window switches to its **NARA Lookup** view, pre-populated with your selection (a new selection replaces a stale query), with six **Query Strategy** options — **Lot file — RG 59**, **Lot file — RG 84**, **Keywords in RG 59**, **Keywords in RG 84**, **Central files identifier (no API key)**, and **General keyword search**. Each carries a one-line note on what it is for, and the panel tells you when the strategy you picked needs a key. The lookup also reads the text around your selection and offers any archival citations it recognizes under **Detected in This Passage** — handy when the citation you want spans more than you selected, or sits a few words from your cursor. Click one to fill the search field; a citation the bundled index already knows is marked with its NAID and needs no key.
...
- **Pointed at, not drawn from** — how often the editors' footnotes send a reader to material *here that FRUS did not print* ("FRUS editors point at unprinted material here N times, across M volumes"), with the citing volumes listed. This is a different body of evidence from every other count on the page, which counts documents FRUS printed *from* the collection — the two are never added. It is bundled and corpus-wide, so it answers before you have indexed anything.
...
The packet is organized around **research targets** — the archival units you would actually ask an archivist about: a central-file class (`762.00`), a lot file, a library collection. Each target itemizes the documents that seeded it **by claim**, kept deliberately apart: documents *published from* the file (with the file or folder designation their source note cites), and footnotes that *cite* the file without FRUS printing from it (quoted verbatim, with a note when the file was inherited from a preceding footnote's "*Ibid.*"). The two counts never merge into one number, because they are different kinds of evidence. A footnote citation is given by the number the **volume printed** for that footnote — the number on the page, whatever its numbering does across a chapter or inside an attachment — and a footnote the volume never numbered says so rather than being given one. Every seeding links to its document on history.state.gov.

What the packet delivers, grouped by repository:

1. **Visit-planning links** — each repository's own visit-planning and finding-aid pages. A link the app has not re-checked recently carries the date it was last checked and a note to confirm current guidance; a link nobody verified is left out rather than printed undated. Only the National Archives at College Park and the presidential libraries the corpus cites carry curated facts; other repositories appear without a links block rather than with guessed ones.
...
3. **The advance inquiry** — a draft email per repository, with the confirmed address and inquiry email for College Park and your project's research question as the editable topic sentence. Citations the app could not resolve appear as **help-me-locate lists quoting each citation verbatim** — source notes and footnote citations each under their own ask, because NARA's own advice is that poorly described records are resolved by writing ahead, not at the reading-room desk — and a divided lot becomes a **question for the archivist**, which is what it is. Each repository's draft has its own **Copy** in the Options menu, ready to paste into an email. A **presidential library** is a repository like College Park: it heads its own section, with its visit-planning and finding-aid pages and its targets, is counted in the packet's header and the plan's summary, and gets its own draft — and because the app holds no confirmed address or email for any library, that draft says so and sends you to the library's own pages for the current contact rather than inventing one. What the app cannot place at any repository — a records center whose holdings may since have moved, a foreign archive, a citation it could not read, or a repository the app has no curated entry for, such as the Library of Congress, the National Defense University or a university library — gets a **Confirm before you travel** list instead, with NARA's advice to write, phone, or email ahead. The list appears in the full-plan export; a single-repository export leaves it to the full plan, and says so. A target you exclude leaves the list, and the coverage report counts it as excluded.

A **coverage report** travels with every export, scoped or not — it is not optional, because an empty channel with no caveat reads as a clearance: how many targets resolved, how far the footnote scan reached (on a pre-1946 reading list an empty footnote channel is the filing practice, not a gap — lot files and presidential libraries are post-war practice), the digitized-substitute denominators, and how much of the restriction picture is actually measured.

The **Options** menu can also scope the export to a **single repository** (College Park, or any presidential library the plan reaches — a repository all of whose targets you excluded is not offered, here or under **Copy**) — a self-contained artifact for one archivist, with the coverage report still describing the whole plan — and can append NARA's citation forms for your packet's series types ("Citing Foreign Affairs Records", quoted verbatim and attributed as NARA's; your publisher's rules govern). The appendix is off by default. For a Central Decimal File number written with a date — `611.93/12–854` is 8 December 1954, the form of almost every decimal file number cited in a document from 1945 on — it quotes NARA's date-numbering example and fills in the file's filing period (1950–1954), unless the number's month and day are the document's own under another year — a misprinted year, as in `740.0011 EW/8–2045` on a document of 20 August 1943 — when it leaves the period for you to fill; a number without a date gets NARA's consecutive-numbering example.
...
Most of the plan's controls sit on the targets themselves: right-click a target to **Set tier**, **Edit Note…**, or **Include in Packet** / **Exclude from Packet**, and orphaned rows collect under **Stored targets**, where **Remove Stored State** is the only way to drop one. The **Filter** menu narrows by repository, tier, claim (**Drawn from** / **Pointed at**), or included-only; **About research targets** explains the two claims and notes that unprinted-reference footnotes sit on about 4% of documents, so a thin pointed-at list is sparse data, not a failed scan. The ⋯ menu also holds **Priority Tiers…**, **Duplicate**, **Re-seed from Project**, **Delete**, and **Export packet** — deleting a plan removes it from your other devices too, after sync. The packet's **Options ▸ What to Include** toggles — **Repository visit-planning links**, **Target list**, **Inquiry email drafts** — persist with the plan, and the **Inquiry topic sentence** field, seeded from the project's research question, is what the drafts send. Long claim lists print their first eight seedings and then say how many more the app holds. Because targets derive from indexed volumes, the plan and the packet both state how many seeding documents this Mac has indexed. The window has no keyboard shortcut.

![The Archives Visits window with a plan's packet open from Export packet — a research target's two claim lists, documents published from the file and footnotes that cite it.](screenshots/macos/trip-packet.png)
...
![Corpus Analytics — a term-frequency chart for "Berlin" by year, 1944–1975, each bar color-coded by source volume with a legend.](screenshots/macos/analytics.png)

Type a term (the full search syntax applies, including quoted phrases) and configure:

- **Group by** — under **Over time**: Decade, Year, Month, Day; under **Broken down by**: **By Subseries** or **By Volume**. The time dimensions chart frequency over time; By Subseries and By Volume break the same query down by where in the corpus it appears (using the same publication-era buckets as the Corpus Browser). On a Subseries or By Volume chart, clicking a bar drills into a Search scoped to that slice.
...
- **To the documents** — click a bar, point, or table row and **View N documents** opens the Search window pre-filtered to that term and period. Never quote a spike you haven't read into.
- **Keep the query.** The toolbar's **Save query** star keeps the current terms, grouping, year range, and scope; the **Saved queries** menu beside it lists saved and recent queries, so a chart you built for a chapter is one click to rebuild.
- **Dispersion.** A footnote under a time chart states how many indexed volumes the matches spread across and, when three volumes hold a quarter or more, how concentrated the term is — a term everywhere and a term in one compilation look alike on a bar chart.
...
- **Lenses.** Chips narrow the cloud to a kind of term: All terms, People / Places / Organizations (recognized on-device), Topics / Actions / Descriptors (nouns / verbs / adjectives), Concepts (abstract ideas like *sovereignty* or *deterrence*), or Sentiment (positively and negatively charged words, green and red). A scope without enough of a given kind of term says so, and a lens that keeps nothing from documents it did read says what it looked for. The People, Places, Organizations, Topics, Actions and Descriptors lenses depend on the Mac's own language analysis; if that analysis is not working, the lens says it is unavailable instead of drawing an empty cloud and names the lenses that still work (which depends on which part of the analysis failed), and quitting and reopening the app may restore it. When the analysis is not reducing words to their dictionary forms, All terms, Concepts and Sentiment still draw, but they count each word as printed — *negotiation* and *negotiations* become two words — and the cloud says **Counted as printed** under its title; so do its CSV, PNG and PDF exports, a comparison column, and the cloud in a collection export. A cloud counted that way cannot be compared with the corpus reference, which was counted in dictionary forms, so Distinctive is withheld for the whole cloud rather than word by word; the Collocates reading steps aside for the same reason, and a related document's shared-word chips are left out.
...
![A scope's Word Cloud in Distinctive mode — the Concepts lens for a Soviet Union 1945-1950 scope, with the eligibility line beneath the controls.](screenshots/macos/word-cloud.png)
...
- **Most-Mentioned People** — a ranking of the people named in the most documents within your year range, switchable between chart and table; narrow the **year range** to compare the early Cold War with détente.
...
**Network** draws a **co-mention ego-network**: a focus person at the center, surrounded by the people they are most often mentioned alongside — a person's immediate "cast" at a glance. The focus person's name is always drawn under the center, on a patch of background over anything beneath it. The other names are drawn where they fit, in this order: the partner shown in the panel beside the graph, then the others by shared documents. A name that would run into another name, the focus person's patch or another node goes unlabeled; hover over a partner or click it to see its name in the panel. A long name is cut at a word break and ends in an ellipsis. The **Focus** line above the graph names the person at the center: **Explore connections** — in the panel, or on a node's right-click menu — and the graph's **Back** move it with the graph, and choosing someone in its **Set focus person…** field always re-centers the graph on them, even the person it opened on.
...
![Cross-Reference Analytics — the Volume Citation Heat Matrix for 1961–1963, each cell shaded by the cross-references running from one volume to another.](screenshots/macos/crossref-analytics.png)
...
- **Show** — **Named collections** (a body of records with a custodian, like the Johnson Library's National Security File) or **Central-file classes** (a subject heading inside State's filing system, like `763.72` for the European War or `POL 27 VIET S`). Different kinds of thing, never mixed in one chart. Classes rank at one depth — decimal numbers stand for themselves, subject-numeric designators group to category and number (`POL 27`), with **Inside these families** opening any grouped row to the exact designators a pull slip needs. Where the bundled schedules cover the era — the decimal file from 1910 to 1963, in three tables because the Department renumbered its classes in 1950, and the subject-numeric file from 1963 — a class reads in words: `793.94` as *China and Japan*, `POL 27 VIET S` as *Vietnam, South — Military Operations*, with the number kept because the number is what a pull slip needs. An era straddling a renumbering, or a class neither table names, stays bare, deliberately: a confident wrong gloss on an archival citation is worse than a plain number you can look up. A decimal country number often named several places — `11g` is three Virgin Islands — so a gloss may end **and N others**; click it for the list.
...
Semantic Analytics tracks none of the above: it measures how the *language* sits. Every document in the series — all 314,571 — is placed on a single map by the shape of its wording, so documents that read alike land near each other whether or not they share a volume, a date, or a citation. Regions are named by the vocabulary that distinguishes them, so you can read the map before you touch it. The placements come from an experimental machine reading of the text — treat the map as a hypothesis generator, not a finding.

- **Color by** Regions, Era, Downloaded (what is on this Mac), or **Provenance** — the last showing each volume's plurality source-note category, with its caveat stated under the lens: a plurality, not a majority, for 73 of the 499 volumes it colors; volumes with fewer than ten notes, and the 30 the aggregate does not cover, take the gray **Too few source notes** color rather than a guess. *Other / Unclassified* is a category in its own right — a note the parser could not place.
- **Nearest in language** on a document's card lists the documents whose wording sits closest — drawn only from volumes on this Mac, and the card says so. A region's card explains that its label is the sample's most distinctive words, not a subject heading. **Zoom In** / **Zoom Out** / **Fit the Map** (⌘+ / ⌘− / ⌘0) and a **Point size** option live in the toolbar.
...
![The semantic map colored by Regions — region labels, the explainer above, and the caveat line beneath the map.](screenshots/macos/semantic-map.png)
...
- **Reading the list.** **Oldest first** / **Newest first** flips the order; **Hide chart** collapses the distribution; a date section over its cap offers **Show all N documents on this date**. Documents with no machine-readable date are not shown, and the footer says so.
...
Open it from **Help → FRUS Research Guide**; it opens in its own window you can keep beside your work. Contextual links into it also appear where guidance is most useful — **Learn About Source Notes** in Source Explorer, **Learn About NARA Lookup** in the lookup view, **About Archival Sourcing** in Archival Analytics — and its internal links open in the embedded browser (16.5), so you never lose your place. Its pages fall into three groups: **About FRUS** (*The Official Record of American Foreign Policy*, *163 Years in Progress*, *Understanding What You're Reading*, *Using FRUS for Research*), **Using the app** (*Finding What You Need in FRUS Explorer*, *Seeing the Bigger Picture in FRUS Explorer*, *Working With Documents in FRUS Explorer*), and **About the Series** (the four dashboards, 16.4).
...
**Shared controls**: a **Scope** control (whole series, a subseries grouped into decade submenus, one of your volume scopes — counted at manifest grain here, downloaded or not, since these dashboards read bundled series-wide data — or a detected topic), an editable **Year range** (1861 to the current year; coverage-based charts clamp their axis at 1993, production-based ones run to the present; **Reset** clears scope and range together), a per-chart **View as table** pop-up (a native Table on macOS) with **Copy CSV**, and a per-chart **Export** menu offering CSV and figures with the same methods discipline as 15.8. Each dashboard carries an **About these figures** note stating exactly what it measures and its limits.

### 16.5 The Embedded Browser

From onboarding, About, and the Research Guide, an external link opens in a built-in browser sheet rather than launching Safari; Source Explorer's NARA Catalog and finding-aid links open in your default browser. Dismiss the sheet and you are exactly where you were, with your document, search, or guide content untouched behind it.

---

## 17. Settings Reference

Open Settings with **⌘,**. The window is a sidebar of panes in four groups — **Library**, **Research**, **Reading & Search**, and **System** — with sub-screens opening as sheets. Type in the sidebar's **Search Settings** field to filter the panes by name or by what they do — "stop words" finds Word Cloud, "api key" finds Connections.
...
| | **Search** | Default scope, default document type, result snippet length (1–10 lines), and **Classification Corrections…**, where every document you reclassified is listed with a per-row Undo |
...
One destination for the corpus on this Mac. It opens with a **Storage used** bar split into **XML**, **Index**, **Summaries**, and **Vectors**, a status line, and the two ways in — **Download from GitHub…** and **Sideload XML File…**. Then: **Downloaded Volumes** (per-volume re-index and remove); **Needs Attention** (only when volumes were interrupted mid-index); **Keeping Current** (Check for Corrections; Refresh Available List — updates preserve your notes, highlights, tags, and summaries); **After an Update** (what volume updates have changed since each volume was last indexed — *No changes waiting* when there is nothing, and otherwise the volumes whose changed documents carry your research, each with a **Mark Reviewed** that asks before it stamps the whole volume); **Storage & Index** (**Free Up Space…**, which lists only volumes with nothing of yours attached and that the app can download again, never-opened first and then least recently opened, and asks first; **Index Remaining**; **Rebuild From Scratch** — none of the three touches notes, highlights, or tags); **Semantic Vectors** and **Natural-Language Search** (Section 5.3a); and **Advanced** (index health with **Check Integrity** and **Copy Research-State Record**, which copies the app build, index versions, indexed volume list, and vector provenance as JSON for a bug report; the **Spotlight Index** rebuild; and **Concurrent downloads**, 1 to 6, default 4).
...
**Reports → Broken Cross-References** exports the corpus-wide list of unresolvable cross-references (Section 8.2) as CSV or JSON. **Diagnostics** holds the redacted iCloud **Sync Log** (event types, timing, and error codes only — never your content; it stays on this Mac), **Semantic Match Feedback** — the verdicts you recorded on Related Documents' semantic rows (Section 8.4), with counts of verdicts recorded and marked helpful, **Prepare Feedback File** / **Share Feedback File** to send them to the developer, and **Clear Recorded Feedback** — and the **iCloud Schema** status (whether iCloud has been told about everything this version saves — an "Update pending" is a problem with the app, not your account, with **Copy Report** for a bug report). **Recovery** is a ladder ordered by cost, each rung stating what it deletes:
...
| **Erase Everything…** | Its own screen, behind two confirmations | Downloaded volumes and the search index, and every note, tag, collection, project, and recorded session — plus saved searches, working corpora, volume scopes, archive visit plans, leads, and person-identity and document-classification corrections — on every device |
...
- **Chart shares, not raw counts**, where eras differ in size — the **% of documents** toggle (15.1) — and check the **Documents** versus **Occurrences (stems)** distinction before calling a topic dead (the `"Article 43"` example in 15.1 is the cautionary tale).
...
| Action | Shortcut |
|--------|----------|
| Open Search window | ⌥⌘F (Find ▸ Search…) |
...
| Open Project Home (active project) | ⇧⌘P — disabled in Global Context |
| Print the document you're reading | ⌘P |
...
| Action | Shortcut |
|--------|----------|
| Show or hide the Research rail (Document ▸ Show Research Panel) | ⌘⇧R |
| Add a research note | ⇧⌘N |
| Highlight the selection in yellow (other colors: Document ▸ Highlight Selection) | ⇧⌘H |
| Find in the document | ⌘F |
| Find next / previous match | ⌘G / ⇧⌘G |
...
| Bold / italic / underline in a prose block | ⌘B / ⌘I / ⌘U |
| Toggle the selected entry's inspector | ↩ |
...
**Menu commands without a shortcut**, all in Section 4.1a: Search Tips…, Open Document in New Window, the Analytics menu, Archives Visits, New Project… and Switch Project, Window ▸ Cross-Reference Graph and Source Explorer, and the Collection menu's heading, prose, highlights, apparatus, sort, and settings items.

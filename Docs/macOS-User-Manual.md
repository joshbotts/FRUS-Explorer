...
| Prefix wildcard | `negoti*` | negotiate, negotiated, negotiating… (suffix wildcards like `*tion` are not supported). The prefix is matched against word stems, so keep it short — `negoti*` finds *negotiations*, but `negotiat*` finds nothing |

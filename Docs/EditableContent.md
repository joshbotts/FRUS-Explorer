...
<!-- SOURCE: FRUSExplorer/Theme/FRUSTheme.swift | FeatureInfoButton.corpusAnalytics FeatureInfoItem | lines: 214–215 | key: analytics.info.metric.body | shared: iOS+macOS (single edit point) -->
...
#### Multiple words
<!-- SOURCE: FRUSExplorer/Theme/FRUSTheme.swift | FeatureInfoButton.corpusAnalytics FeatureInfoItem | lines: 218–219 | key: analytics.info.multiword.body.v2 | shared: iOS+macOS (single edit point) -->
...
Note: replaces `analytics.info.multiword.body` (#1297), whose "NOT, or a leading -, excludes a term" said nothing about where an exclusion on a word applies: to the words it is typed with, wherever it sits among them, and never across OR. (Excluding a whole group is different — it reverses every mark inside the group — which the user manuals' §7.2 explains.) Reworded in place before shipping for #1297 round 1: its closing "All of this works exactly as it does in the Search box" promised a disclosure Analytics does not make — Search's Query Inspector marks a left-out exclusion-only alternative NOT APPLIED, and this chart has no inspector — and said nothing of what `=` does under parser 6.3, which applies the mark only where every match must contain the word.

#### Phrases
<!-- SOURCE: FRUSExplorer/Theme/FRUSTheme.swift | FeatureInfoButton.corpusAnalytics FeatureInfoItem | lines: 222–223 | key: analytics.info.phrase.body | shared: iOS+macOS (single edit point) -->
...
#### Stemming
<!-- SOURCE: FRUSExplorer/Theme/FRUSTheme.swift | FeatureInfoButton.corpusAnalytics FeatureInfoItem | lines: 226–227 | key: analytics.info.stemming.body | shared: iOS+macOS (single edit point) -->
...
<!-- SOURCE: FRUSExplorer/Theme/FRUSTheme.swift | FeatureInfoButton.corpusAnalytics FeatureInfoItem | lines: 230–231 | key: analytics.info.dating.body | shared: iOS+macOS (single edit point) -->
...
Note: the detail line under a term tagged EXCLUDED. Replaces `search.inspector.excludedDetail` (“excluded — documents containing this are removed”), which never had a block here; re-keyed for #1297, because an exclusion is not a removal from the whole result set: it applies where the MATCH expression above applies it. A `-` or NOT on a word never reaches across OR, but excluding a whole group reverses every mark inside it, and that can join the alternatives of an OR typed inside the group — `NOT (cold OR -korea)` removes cold from korea’s matches — so the line points at the expression rather than at the words typed beside the term.

...
You can colour the map by region, by coverage era, by volume, or by what is downloaded on this device; tap a point to see which document it is and open it; draw a lasso around an area to keep everything inside it as a working corpus; or pick two documents as poles and lay the whole series along the axis between them — which replaces the vertical axis with each volume's coverage year.

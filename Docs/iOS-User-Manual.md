...
On an iPad with room for two panes — in landscape, for instance — the root stays on the left as a list while the level you open fills the right. The row or tile you opened it from stays highlighted in the list, however deep you go inside it, and VoiceOver reads it as selected. A document is the exception: it takes the whole width unless the window also has room for the Research rail beside it (Section 4.2), which on a 13-inch iPad means landscape with the tabs as a floating bar. With the tabs in the sidebar, the navigation bar across the panes names the level you opened, with your project's research question beneath it (Section 10.2).

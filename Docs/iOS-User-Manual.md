...
Open the **Collections** tab and tap **New Collection**. The editor is its own screen, titled with the collection's name (**New Collection** until you give it one), and every edit **saves as you go** — there is no Save button. That holds in **Collection settings** and in a heading's **Section defaults** too: what you type there is on the collection at once, so leaving by Back, by another tab, or by closing the app loses nothing, and a description, subtitle or author line changed in one shows in the other. (Backing out of a brand-new collection you never touched discards it.)

...
- **Download and index** any subset of the corpus — one volume, a publication era, or all 553 volumes — for instant full-text search on your device, with no network connection needed afterward.
...
All of your research data — notes, tags, collections, highlights, projects — syncs automatically across every device signed into the same iCloud account, so a note made on your iPad in the library is on your iPhone on the bus and on your Mac at your desk. When it cannot, a bar along the bottom of the screen says why — **Local Only** (iCloud could not start, so new work stays on this device), **iCloud Account Issue** (not signed in, or the account is restricted), **iCloud Sync Zone Missing**, or **iCloud Sync Failed** — and its **Details** button opens Settings, where the iCloud Sync row explains it. The bar says nothing while sync is working, and steps aside while the keyboard is up or a download or indexing banner needs the space.
...
| Option | Description |
|--------|-------------|
| **Corpus** | All 553 volumes in the bundled catalog (about 3.3 GB; downloading takes a while depending on your connection and free storage) |
...
![Onboarding on iPad — the Add Volumes step (Corpus, Subseries or Volume) over the word-cloud backdrop, here showing its Concepts view.](screenshots/ipad/onboarding-volumes.png)
...
5. **Open the Research rail.** Tap the rail toggle. On iPad the rail slides in as a panel beside the document; it opens with a grid of seven tiles — **Cite**, **Word Cloud**, **Sources**, **Graph**, **Related**, **On the Map**, **Share** — above expandable **Topics** (when the document has any), **Summary**, **Notes**, **Tags**, and **Collections** sections. This rail is the per-document research surface; everything you do *about* a document (rather than *in* it) starts here.
...
![Browse on iPad in landscape — the adaptive tab sidebar, with the active project in its Projects footer, beside the Browse root.](screenshots/ipad/sidebar-landscape.png)
...
| **On the Map** | The semantic map centered on this document, among the documents whose language is most like it (Section 15.6) |
| **Share** | Zotero send, Zotero-file export, and citation sharing (Section 11) |

Below the tiles sit expandable accordions — **Topics** (the document's detected subjects, most distinctive first, shown only when there are any; in a main window each opens the Topic index, Section 6.2a), **Summary** (AI summaries, Section 13), **Notes**, **Tags**, and **Collections** (Sections 9 and 12). The Topics caption says what it is: detected automatically from the text, not editorial subject headings, so some are wrong. On iPad the rail header also carries an **Open in New Window** icon for Stage Manager (Section 4.5).

The rail header's **ⓘ** button opens a **Document tools** popover explaining each of the seven tiles — and, at the bottom of that popover, carries the document's **Classification**: whether the app treats what you're reading as a *document* or an *editorial note*, with the control to correct it. FRUS's own tagging is occasionally wrong — a substantive document marked as an editorial note, or the reverse — and because the app trusts that tagging, the mistake reaches the type badge, search's document-type filter, counts, and exports. **Reclassify as Document…** / **Reclassify as Editorial Note…** records your correction: the body restyles immediately, every filter and badge follows it on all your devices, and the popover then shows what FRUS tags it as so the disagreement stays visible. Both track FRUS's own later corrections: if the Office of the Historian fixes the same tagging, the popover says so — *FRUS now tags this the same way, so your correction no longer changes anything* — and **Restore FRUS's Classification** puts the new value back, not the reading recorded on the day you disagreed. The correction is fully reversible — **Restore FRUS's Classification** in the same popover, or manage all of your corrections (with per-row Undo) under **Settings → Search → Classification Corrections…**. One caveat the confirmation dialog also states: the bundled series-analytics dashboards are computed from the published corpus and cannot see your corrections.

![A document on iPad in landscape with the Research rail open as a trailing inspector — the RESEARCH header, the seven-tile grid (Cite · Word Cloud · Sources · Graph · Related · On the Map · Share), and the Topics, Summary, Notes, Tags and Collections accordions beside the text.](screenshots/ipad/research-rail.png)
...
- **Documents in their own windows.** Tap **Open in New Window** in the Research rail's header to pop the current document out, then arrange it beside other FRUS Explorer windows or other apps — two documents side by side, or a reference document beside the one you're annotating. A popped-out window reads on its own: cross-references and edge-taps navigate *inside* it, with its own Back button. Its rail still lists the document's **Topics**, but they do not open the Topic index there, and a person's sheet shows how many indexed documents mention them without offering **Find all mentions** — the index and the search each open in a main window's tab, which a popped-out window does not have.
...
- **Independent tabs.** Each main window keeps its own tab selection: switching to Search in one window doesn't switch the others, and an action that hands you to another tab (a "Find all mentions" search from a person sheet, say) brings the right tab forward in one window only.
...
![Stage Manager on iPad — a document window beside a Related Documents window, with the main window behind.](screenshots/ipad/stage-manager.png)
...
**The indexing banner.** While volumes download and index, a banner above the tab bar shows progress for the current volume and, when several are queued, your position in the queue with an estimated time. Before the first volume has finished transferring there is nothing to measure yet, so the banner reads simply *"3 downloads queued"* — the count includes the one currently coming down, and it is replaced by the progress banner as soon as indexing starts. The banner tracks the whole **queue**: download a subseries and it appears once, when indexing begins, and stays until every volume is searchable — the volume named inside it changes as work moves along. When the queue finishes, the banner becomes a brief summary card ("*27 volumes ready to search*") offering to search what was added. A dot badge on the **Settings** tab marks downloaded volumes still waiting to be indexed. Tapping a name mentioned in the banner — a person discovered during indexing — can jump straight to a search for that name.
...
On an iPad with room for two panes — in landscape, for instance — the root stays on the left as a list while the level you open fills the right. The row or tile you opened it from stays highlighted in the list, however deep you go inside it, and VoiceOver reads it as selected. That includes **Continue reading**: while the document it reopened is open beside the list, the row keeps naming that document, even after you follow a reference out of it or read something else in another tab. A document is the exception: it takes the whole width unless the window also has room for the Research rail beside it (Section 4.2), which on a 13-inch iPad means landscape with the tabs as a floating bar. With the tabs in the sidebar, the navigation bar across the panes names the level you opened, with your project's research question beneath it (Section 10.2). **Back** brings the level beneath back with what you set on it: in Archives, the lens you chose, the eras and groups you closed, and the collection search; in a collection, the lists you expanded with **Show all**; in All Volumes and Editors, the search. The list on the left keeps its search too, when a document has taken its place. Turning the iPad across the width where the panes appear keeps all of these. The level itself is drawn again, though: it opens at the top of its list, and a collection whose sub-series you opened in Archives' Collections list shows them closed again. Choosing a door again from the list on the left opens that level afresh.
...
- **Subseries** — the classic era-by-era hierarchy (Section 6.1a). While the hierarchy is narrowed to one of your scopes with **Browse Within This Scope**, this tile says so: it names the scope and counts that scope's volumes (Section 6.1e).
...
![The All Volumes catalog in Published order — the Title / Published / Era / Length control above decade headers, newest first.](screenshots/ipad/browse-all-volumes.png)
...
![The Administrations index — each presidency with its volume and document reach, the post-corpus presidencies dimmed at the end.](screenshots/ipad/browse-administrations.png)

![The Truman administration's volumes, largest share of the term's documents first — each row with its document count and share, and a note where a volume also falls under a neighboring administration.](screenshots/ipad/browse-administrations-truman.png)
...
![The Editors index — letter sections, each editor with a volume count, and a "2 spellings merged" caption under a name recorded two ways.](screenshots/ipad/browse-editors.png)
...
![Working Corpora — each corpus with where it came from, when, and whether the capture was complete.](screenshots/ipad/browse-working-corpora.png)

![A working corpus's documents — the amber coverage line, titled rows for indexed volumes, and gray identifier rows with a Download button for a volume this device does not have.](screenshots/ipad/browse-corpus-drill.png)

### 6.1g Archives

Nearly every FRUS document carries a printed **source note** saying which file it came from, and the **Archives** door browses the series by that archival record — three lenses, side by side:
...
- **Collections** — the same archival collection index the Source Explorer uses (every collection in the bundled cross-volume authority, searchable), grouped by **repository**, by National Archives **record group**, or not at all, opening each collection's full record: its NARA catalog link, related collections, when the series cited it, and every citing volume — tap one to browse it without losing your place.
- **Classes** — the State Department's central-file classes, divided by the **filing schedule** in force: the three decimal-file schedules (1910–49, the 1950s, 1960–63) and the two subject-numeric arrangements that followed. The division is the point — the same number means different things under different schedules, so a class can appear in two sections reading two different ways. Where the Department's own filing manuals give a reading, the class shows it in words; each opens the volumes that cite it.

**Collections** and **Classes** each have a **Sort** menu: order by **document count** or by name — a collection's name, or a class number in the file's own order — **ascending or descending**. Choosing name starts at the beginning of the alphabet or the file; choosing document count starts with the most. **Collections** add a **Group** menu: repository, record group, or **Ungrouped** — one list ranking every collection against every other, which is the only way to see the most-cited collections across all repositories. Groups and filing eras **collapse**: tap a header to close or open it, or use **Collapse All** / **Expand All** beside the menus. A search shows every match even inside a closed group, and the groups you had closed come back when the search clears. Sort and grouping are remembered on this device; which groups are closed is not. Three details matter when you read the result: by class number, decimal numbers file digit by digit, so *711.11* comes before *711.2*; collections whose citations name no record group — nearly every presidential-library collection among them — are listed together at the end; and a collection cited only in a volume's front matter, never under a document, shows its volumes but no document count.

The lenses sit **beside** each other on purpose: the app has no reliable mapping from a collection to a provenance type, so it shows both truthfully rather than nesting one under the other. The screen also states its limits up front — the counts describe the notes printed under documents (not every document), only about 28% of sourced documents name a *collection* (most cite a central-file number, which is what the types lens holds), and the ~50 volumes that print no notes at all — mostly the pre-1906 annuals — can't appear here. For deeper archival analysis (era rankings, co-citation networks, central-file classes), the **Archival Analytics** tool in the Analysis menu remains the instrument.

![Archives, Provenance Types lens — the ten kinds of file FRUS drew on, each with its counts.](screenshots/ipad/browse-archives-provenance.png)

![Archives, Collections lens — grouped by record group, with the Group and Sort menus and a collapsed group.](screenshots/ipad/browse-archives-collections.png)

![Archives, Classes lens — central-file classes under their filing schedule, sorted by document count, each read in words where the manuals give a reading.](screenshots/ipad/browse-archives.png)

![A collection's detail, reached by push — its identity, its other spellings, and the NARA catalog record it resolves to, offline.](screenshots/ipad/browse-archives-collection-detail.png)

![Further down the same detail — when the series cited it, the unprinted material editors pointed at, and its citing volumes.](screenshots/ipad/browse-archives-collection-citing.png)

### 6.1h Clusters

The **Clusters** door browses the corpus the way a language model read it: 171 groups of documents whose language reads alike, computed from the text itself rather than chosen by an editor. This is the same grouping the **semantic map** (Section 15.6) draws as colored regions — here it is a browsable list, largest cluster first.
...
![The Clusters index — each cluster's sampled label, document count, and era histogram, under the three honesty rules.](screenshots/ipad/browse-clusters.png)

![A cluster's documents — the era breakdown and coverage line, See on the semantic map, Save as Working Corpus, and its documents grouped by volume.](screenshots/ipad/browse-clusters-detail.png)

![The semantic map focused on a cluster after "See on the semantic map".](screenshots/ipad/browse-clusters-map.png)
...
![The Topic Index narrowed to one topic area — the area chip with its ✕, and each topic with its reach across the series.](screenshots/ipad/topic-index.png)
...
**All «area» topics** (for example *All Cold War topics*) goes back to the index narrowed to the topic's own area, so a reader who found one Cold War topic can see its neighbors without scrolling all 491. The narrowing shows above the list as a chip — *Topic area: Cold War — 6 topics* — with a ✕ to return to the full index. Taking the door clears any search you had typed, so the whole area is listed; search within the area afterwards and the chip says how many of its topics are showing — *Topic area: Cold War — 1 of 6 topics*.

Reach it five ways: **Browse ▸ Topics** (beside People), a topic in the **Topics** section of a document's Research rail (Section 4.2), the **Browse this topic in the index** button on any topic chip's pivot sheet, **Browse all topics** in the Subjects section of a search's Facets panel, or **Browse all topics…** in the scope menu of an analytics dashboard or an *About the Series* page. Each of the last two closes the panel or sheet it was on and brings the Browse tab forward, so the index arrives in front of you rather than behind what you were reading; a rail topic in a main window brings Browse forward too, closing the rail on iPhone, where it is a sheet. In a document read in a window of its own — popped out (Section 4.5), or opened inside the cross-reference graph, semantic map or Chronology window — the rail's topics are listed without opening the index, because the window has no Browse tab to show it in. **On iPad the analytics dashboards open as their own windows, which have no Browse tab to bring forward, so they do not offer the item there** — use the Facets panel, an *About the Series* page (which is a sheet on both), or Browse ▸ Topics instead. Wherever you arrive from, the Topic index replaces where you were in Browse rather than stacking on top of it. **Browse ▸ Topics** always opens the whole index, clearing any area chip and search — on iPad even when the index is already showing beside the list. Opening a covering volume and going Back returns you to the index as you left it, with its area and your search in place.
...
![People browser — a single alphabetical list of reconciled identities with role · active-years subtitles and corpus-wide mention counts.](screenshots/ipad/people-list.png)

Tap a person for their **detail sheet**. A **reconciled-identity seal** there marks a person matched to the bundled name-authority data, and a reconciled person's **life years** sit under the name, beside the role the Office of the Historian's people register gives them — the register's own years, with a year it lacks filled from the Principal Officers and Chiefs of Mission register. Those say when the person lived; the **Active** row below them is the volumes' span, as in the list.

![Person detail for "Kissinger, Henry A." — the reconciled-identity seal, mention count and active years, subjects, and Find all mentions.](screenshots/ipad/people-detail.png)

![Further down the same sheet — the Career section and the underlying volume records, each with its Separate action.](screenshots/ipad/people-detail-records.png)
...
**Search Tips.** A short reference to this syntax is always a tap away: **•••** → **Search Tips** in the search actions bar, or, in Keywords mode, the **Search tips** link on the Search screen before you search. When a query cannot run, or runs narrower than you typed, a **Search tips** link also appears under the Query Inspector (Section 7.3). On iPad with a keyboard, **Find → Search Tips…** in the menu bar opens it from any tab. Each tip shows the syntax exactly as you type it and what it does, followed by what the date filter does and where the search scope is set. In Meaning mode the sheet shows no syntax: it says instead that a meaning search reads your words as a whole, so none of these marks has a special effect. The words reach the model as you typed them, and only the filters you set narrow a meaning search, so an `=` there narrows nothing.

[SCREENSHOT: iPhone — the Search Tips sheet at half height over the Search screen, showing the first rows]

| Syntax | Effect |
|--------|--------|
| `word1 word2` | Both words, in any order |
| `"word1 word2"` | The words together, in that order |
| `word1 OR word2` | Either word. `OR`, `AND` and `NOT` work in any case; quote one (`"and"`) to search for the word itself. Words side by side bind tighter than `OR`: `cold war OR peace` finds *cold* with *war*, or *peace* |
| `-word` or `NOT word` | Excludes documents containing the word from the terms it is typed with, wherever it sits among them — `-korea cold` is the same search as `cold -korea`. The one exception is `NEAR(…)`: only `NOT NEAR(…)` excludes it |
| `(word1 OR word2) word3` | Parentheses group terms, and nest up to 32 deep — a query nested deeper is not run |
| `-(word1 OR word2)` or `NOT (word1 OR word2)` | Excludes everything the group matches. The `-` must touch the parenthesis: on its own a `-` is ignored, so `cold - (korea OR vietnam)` searches *for* the group |
| `negoti*` | Prefix wildcard: negotiate, negotiated, negotiating… The prefix is matched against word stems, so keep it short — `negotiat*` misses *negotiations*, which `negoti*` finds |
| `NEAR("military guarantee" Europe, 30)` | Both operands within 30 words of each other, in either order |
| `=containment` | The word with stemming turned off — not *contain*, *containing*, *container*; capitalization still does not matter |

**Quotation marks.** Straight (`"cold war"`), curly (`“cold war”`, `„cold war“`) and guillemet (`«cold war»`) quotation marks all make a phrase, so a phrase typed with Smart Punctuation on, or pasted from a document, is searched as the phrase it is. A phrase cannot hold quotation marks of its own: every mark opens or closes a phrase, so `"the “iron curtain” speech"` searches its four words separately. Leave the inner marks out — `"the iron curtain speech"` still matches a document that prints them.
...
![Search in Meaning mode — the Keywords | Meaning control, the disclosure strip, and ranked matches each carrying its semantic-match score.](screenshots/ipad/semantic-search.png)
...
- **Following a cross-reference keeps you where you are reading.** From a document opened out of search results, the target opens on the Search tab and Back returns to the document the link was in — your results are still underneath. The same holds in Browse, in the Research and Collections tabs, and inside the Chronology, Citation Lookup, and cross-reference-graph sheets.
- **A document you open inside a tab reads in that tab.** Tapping a document in a Research list or in History, reading from a collection, following a lead or recent document in Project Home (from the Research tab or from Settings), or opening a seeded document in an Archives Visit opens it right where you are, and **Back** returns to the list you came from. A document that arrives from outside any tab — a Spotlight result, a link from another app, Handoff from another device, or a tap in iPad's separate Related Documents, Archival Neighbors, Source Explorer, or Cross-Reference Analytics window — opens in the Browse tab. (The cross-reference graph and semantic map windows open documents inside themselves.)
...
![Related Documents — the This volume / This subseries / All volumes scope, Adjust weights open with a slider per signal and Reset, and ranked rows with their "why related" chips.](screenshots/ipad/related-documents.png)
...
![The cross-reference graph in Network mode at three hops — document nodes, teal archival-citation nodes, and the reference list panel beside the canvas.](screenshots/ipad/cross-reference-graph.png)
...
Tap a category to see the matching documents; tapping one opens it in the document view right here in the Research tab, and **Back** returns to the list.
...
Open the **Collections** tab and tap **New Collection**. The editor is its own screen, titled with the collection's name (**New Collection** until you give it one), and every edit **saves as you go** — there is no Save button. That holds in **Collection settings** and in a heading's **Section defaults** too, and a description, subtitle or author line changed in one shows in the other. Collection settings saves each edit as you make it, so leaving by Back, by another tab, or by closing the app loses nothing. Section defaults puts each edit on the collection at once, so leaving the sheet or the editor loses nothing, and the app saves it with its regular saves. (Backing out of a brand-new collection you never touched discards it.)
...
![The collection manager on iPad — the Contents outline (section headings, a prose block, documents with their Configure pills) beside the live preview.](screenshots/ipad/collections-editor.png)
...
- **Reading from a collection**: long-press a row and choose **Open Document** (tapping the row opens its settings instead). The document opens inside the collection, and **Back** returns to it.
...
From the manager's **add** menu, insert editorial structure anywhere in the order (the same menu also offers **Add to Archives Visit…**, which seeds a plan from the collection's documents — Section 14.8; it stays disabled until the collection has documents):
...
- **Prose blocks** are your connecting commentary, written in a rich-text editor — bold, italic, underline, color, and hyperlinks from the formatting bar above the keyboard — and preserved through every export. This is where a course reader's head-of-section framing lives. A long block shows its opening lines in the outline — at most six — ending in an ellipsis; tap it to edit the whole block, and it goes back to its opening lines when you finish editing (**Done** on the formatting bar). It stays open while that bar's color picker or link alert is up. The introduction in Collection settings works the same way.
...
The app ships eight standard prompts — a general *Standard Summary* plus seven structured ones that return named fields, including *Meeting Record*, *Diplomatic Exchange* and *Relevance Assessment* — and **Settings → Research → Summarization** is where you create your own: a prompt that pulls out the dates and places a document fixes, say, or one that briefs a document against your specific research question. Choose which prompt to use when generating, and manage saved prompts in the same pane. A prompt of your own is not held to the shipped prompts' as-printed rule: if you ask a model to identify someone the document does not name, it will try.
...
![Source Explorer on iPad — the parsed source note, and the NARA catalog record it resolves to.](screenshots/ipad/source-explorer.png)
...
- **Pre-1910 Central Files** — 1906–1910 Numerical File rolls, the pre-1906 country-arranged diplomatic series back through the 19th century (a chapter of correspondence with a foreign legation in Washington, such as *British legation.*, resolves to that legation's Notes to or from Foreign Missions, never to an instruction or despatch), and the chronological runs (Consular Instructions, Notes to and from Foreign Consuls, Domestic Letters, Letters Received, and the Special Agents series — matched by date alone) — resolve from a **bundled index, no API key required**. For a pre-1906 document that prints a **serial** above its text (*No. 428*), the digitized-records section repeats it as a second handle for the rolls — the microfilm images are browsed by eye, and the serial sits on them beside the date — and says whose number it is: **Instruction No.** for the Department's instruction to a post, **Despatch No.** for a post's despatch, and a plain **No.** otherwise — for a note, and for a Department letter that could be either until the register settles it. It is not a NARA identifier and resolves to no catalog record; about one pre-1906 document in ten carries one. A Department letter headed *Mr. Seward to Mr. Dayton* could be an instruction to the U.S. minister or a note to a foreign diplomat in Washington, and the header does not say which. Where the Office of the Historian's register of chiefs of mission shows the addressee as the U.S. chief of mission at that post on that date, **Diplomatic Instructions** comes first, marked **Likely**, with a line naming the minister (*William L. Dayton, U.S. Envoy Extraordinary and Minister Plenipotentiary to France (1861–1864)*), and the Notes to Foreign Missions reel is not offered. Where the register cannot settle it — no one of that name held the post on that date (it allows 90 days after a tenure ends), two people did, the addressee or the sender carries a foreign title, or the addressee is a description such as *the Japanese Minister* — both reels stay, marked **Possible**. The section reads the document's dateline and chapter from the search index, so it gives the same answer however Source Explorer was opened — from Browse, Research, History, a collection or Citation Lookup — and when it cannot check, it says why (the index is still starting, the document is not indexed on this device, it prints no dateline) rather than showing nothing.
...
![The Archives Visit packet — a research target with its two claim lists, documents published from the file and footnotes that cite it, with the Share buttons above.](screenshots/ipad/trip-packet.png)

![The packet's Options menu — the repository scope, a copy of each facility's inquiry draft, and what to include.](screenshots/ipad/trip-packet-options.png)
...
![Corpus Analytics on iPad — four terms charted by year as a share of documents, with the Measure, value and Export controls inline in the toolbar.](screenshots/ipad/analytics.png)
...
- **Lenses.** Chips narrow the cloud to a kind of term: All terms, People / Places / Organizations (recognized on-device), Topics / Actions / Descriptors (nouns / verbs / adjectives), Concepts (abstract ideas like *sovereignty* or *deterrence*), or Sentiment (positively and negatively charged words, green and red). A scope without enough of a given kind of term says so rather than showing a near-empty cloud, and a lens that keeps nothing from documents it did read says what it looked for. The People, Places, Organizations, Topics, Actions and Descriptors lenses depend on the device's own language analysis; if that analysis is not working, the lens says it is unavailable on this device instead of drawing an empty cloud and names the lenses that still work (which depends on which part of the analysis failed), and quitting and reopening the app may restore it. When the analysis is not reducing words to their dictionary forms, All terms, Concepts and Sentiment still draw, but they count each word as printed — *negotiation* and *negotiations* become two words — and the cloud says **Counted as printed** under its title; so do its CSV, PNG and PDF exports, a comparison column, and the cloud in a collection export. A cloud counted that way cannot be compared with the corpus reference, which was counted in dictionary forms, so Distinctive is withheld for the whole cloud rather than word by word; the Collocates reading (Section 7.6) steps aside for the same reason, and a related document's shared-word chips are left out.
...
![A scope's Word Cloud in Distinctive mode — the Concepts lens for a Soviet Union 1945-1950 scope, with the eligibility line beneath the mode control.](screenshots/ipad/word-cloud.png)
...
**Network** draws a **co-mention ego-network**: a focus person at the center, surrounded by the people most often mentioned in the same documents, connection strength reflected in the graph. Re-center on anyone to walk the orbit. The focus person's name is always drawn under the center, on a patch of background over anything beneath it. The other names are drawn where they fit, in this order: the partner you selected, then the others by shared documents. A name that would run into another name, the focus person's patch or another node goes unlabeled; tap a partner to see its name in the panel beside or below the graph. A long name is cut at a word break and ends in an ellipsis. The **Focus** line above the graph names the person at the center: **Explore connections** — in the panel, or on a node's long-press menu — and the graph's **Back** move it with the graph, and choosing someone in its **Set focus person…** field always re-centers the graph on them, even the person it opened on.
...
![Cross-Reference Analytics on iPad — the Scope, year-range and Administration controls above the Most-Referenced Documents ranking by inbound citations.](screenshots/ipad/crossref-analytics.png)
...
- **Lasso** a region (the toolbar toggle) and keep everything inside it as a **working corpus** to search within (Section 7.9) — the capture card states how much of the selection this device can actually search, because the map draws all 553 volumes and your index holds fewer.
...
![The semantic map colored by Regions — region labels, the explainer card above, and the caveat line beneath the map.](screenshots/ipad/semantic-map.png)
...
![Chronology on iPad — the From/To range (autumn 1962), the stacked distribution chart with its per-volume legend, and the date-grouped document list.](screenshots/ipad/chronology.png)
...
![About the Series — Production & Timeliness on an empty install: the publication-lag scatter, colored by era, against the timeliness targets in force.](screenshots/ipad/series-production.png)
...
1. **Build the Archives Visit** (14.8): from Project Home tap **Plan a Visit**, or seed a plan from a collection's add menu (**Add to Archives Visit…**), Source Explorer, or Archival Neighbors. This automates what used to be this recipe's first six steps — the plan resolves every source note and footnote citation into research targets with the RG / entry / series / NAID lines staff ask you to quote, lets you prioritize them into your own tiers, drafts the advance inquiries, and marks restricted series and mandatory substitutes on the targets and documents they belong to.
...
| Plan an archive visit | Research tab → **Archives Visits**; seed from Project Home, a collection, Source Explorer, or Archival Neighbors (14.8) |

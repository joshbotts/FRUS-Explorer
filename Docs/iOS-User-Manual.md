...
| `word1 OR word2` | Either word. `OR`, `AND` and `NOT` work in any case; quote one (`"and"`) to search for the word itself |
| `-word` or `NOT word` | Excludes documents containing the word from the terms it is typed with, wherever it sits among them — `-korea cold` is the same search as `cold -korea` |

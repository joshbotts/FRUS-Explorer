...
On an iPad with room for two panes — in landscape, for instance — the root stays on the left as a list while the level you open fills the right. A document is the exception: it takes the whole width unless the window also has room for the Research rail beside it (Section 4.2), which on a 13-inch iPad means landscape with the tabs as a floating bar. With the tabs in the sidebar, the navigation bar across the panes names the level you opened, with your project's research question beneath it (Section 10.2).

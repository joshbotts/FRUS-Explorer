...
**A search needs a word to find.** A query made only of exclusions — `-korea`, or `NOT korea` — has nothing to search for, and neither does one whose parentheses nest more than 32 deep. The Query Inspector says so as you type (Section 7.3), and if you search anyway, the results area says the query has nothing it can search for and points you to Search Tips. If every toggle under **Filters → Search Scope** is off, the results area says that instead, since there is nowhere to search.

...
| `negoti*` | Prefix wildcard: negotiate, negotiated, negotiating… The prefix is matched against word stems, so keep it short — `negoti*` finds *negotiations*, but `negotiat*` finds nothing |

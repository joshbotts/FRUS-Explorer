...
def main():
    ...
    for i, vol in enumerate(scope):
        ...
        for r in rows:
            ...
            toks = [t for t in toks if t and t not in TITLES and not re.fullmatch(r"(?i)messrs|señor|senor|m|mme|herr|don|general|admiral|baron|count", t)]
            m_ini = None
            if len(toks) >= 2 and toks[0][0].isalpha() and fold(toks[0]) != key:
                m_ini = fold(toks[0][0])
    ...
    hout = {"generated_by": os.path.abspath(__file__), "sample": "1,500 documents per band, random.Random(234), from the TEI-dated documents of the scope",
            "definitions": {

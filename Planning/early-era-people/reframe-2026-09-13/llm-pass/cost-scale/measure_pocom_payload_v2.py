...
text_of, load_windows, live-window rule (±1) by import. Read-only. Writes pocom-payload-v2.json here."""
...
def tally(rowmap):
    ...
    for (v, key), (stripped, n) in rowmap.items():
        ...
        if live:
            lr.append(1); ls3.append(min(3, n)); lm.append(n); ld.append(cw)
    R = len(cand_n)
    def dist(xs):
    ...
    return {"rows": R, "surname_unknown_rows": unknown, "rows_0_live": zero, "rows_1_live": one, "rows_several_live": several,
            ...
            "live_rows": {"rows": len(lr), "snippets_cap3": sum(ls3), "mentions": sum(lm), "desc_chars_in_window": sum(ld)},
            "surface_chars_sum": sum(len(k[1]) for k in rowmap)}
...
json.dump(res, open(os.path.join(HERE, "pocom-payload-v2.json"), "w"), indent=1, ensure_ascii=False)
print(json.dumps({k:(v.get("live_rows"),v.get("surface_chars_sum")) for k,v in res.items() if isinstance(v,dict)}))

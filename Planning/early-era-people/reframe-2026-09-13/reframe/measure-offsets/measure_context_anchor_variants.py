...
Writes summary-context-anchor-%s.json' % os.environ.get('CASCADE_NAME','x') + '."""
import sys, os, json, html, collections
CASCADE = tuple(int(x) for x in os.environ.get('CASCADE','20,10,0').split(','))
...
for store in ('sweep_filtered', 'control_filtered'):
    for lab in ['ALL'] + [b[2] for b in ns.BANDS]:
        ...
        for r in rs:
            ...
            grp[g][r['result']] += 1
            if r['result'] in ('correct', 'wrong', 'placed_footnote_span_in_body', 'unique_unverifiable'): grp[g]['L=%s' % r['L']] += 1
        summ['%s/%s' % (store, lab)] = {g: dict(c) for g, c in grp.items()}
json.dump(summ, open(os.path.join(OUT, 'summary-context-anchor-%s.json' % os.environ.get('CASCADE_NAME','x')), 'w'), indent=1)

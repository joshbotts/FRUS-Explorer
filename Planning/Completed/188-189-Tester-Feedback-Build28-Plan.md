...
| 5b-1 ✅ | **Person Analytics scope** | 189-B | Shipped (#199): subseries/volume scope through all 7 `PersonMentionStore` queries. |
| 5b-2 ✅ | **Cross-Reference Analytics scope** | 189-B | Shipped: year-range bar + subseries/volume scope. **Source-anchored** (count citations made by in-slice/in-era sources; heat matrix stays both-endpoints), decided via a design-panel workflow + user confirm. Date+volume filters on all 5 CA-6 queries, gated so whole-corpus = byte-identical. `claude/189b-crossref-scope`. Decade-as-filter dropped (redundant with year-range); Series dashboards already year-range/administration scoped → 189-B complete. |

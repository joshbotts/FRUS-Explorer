...
- **D9 — Native shareable collection format (`.fruscollection`). Recommended as the flagship Phase 4 target.** A round-trippable file that lets a researcher hand a colleague the *composition itself* — not just a rendered artifact — which the recipient opens as a live, editable collection.
  - **Why it's feasible (the hard part is already solved).** Document references are `volumeId` + `documentId` — stable, universal FRUS identifiers (volume filename + TEI `xml:id`) shared by every user. A reference authored on device A resolves on device B as soon as that volume is downloaded and indexed; nothing device-specific is needed. The bundled `manifest.json` (~552 volumes: title, date range, size, download URL — `ManifestStore`) means an importer can look up any *missing* referenced volume and offer one-click download+index.
  - **Why it's high-value.** It is the **only** format that round-trips the editorial work: section headings, rich-text prose, per-entry/section body-depth overrides, and composition settings. PDF / DOCX / HTML / RIS are all lossy, one-way *products*. Native sharing turns a collection into a portable scholarly object (a syllabus, a briefing packet, a document set) that colleagues can extend.
  - **Design.** Flat, versioned **JSON** (`formatVersion` for forward evolution), extension `.fruscollection`, exported UTI `bottsywattsy.FRUS-Explorer.fruscollection` conforming to `public.json` — emailable / AirDrop / Files-friendly; prose RTF base64-embedded. **Codable DTOs decoupled from the SwiftData models** (never serialize SwiftData ids/timestamps). Serialize: name, note, composition settings, and ordered entries (kind + sortOrder + per-kind payload: document → volumeId/documentId/bodyDepthOverride[/inline note text]; heading → text/bodyDepthOverride; prose → text + base64 RTF).
  - **Import contract.** Regenerate Collection/entry UUIDs, reset timestamps, drop `projectIds` (import into the current project context or none). **Import always succeeds**; documents whose volume isn't present render as "needs download," with a manifest-driven **"Download missing volumes (N, ~X MB)"** action.
  - **Note — not a `CollectionExporter`.** That protocol consumes *resolved* `[CollectionExportItem]` (rendered content) to produce an artifact. The native format instead serializes the *source* (references + composition + structural entries) straight from the `Collection` model — no content resolution, no volume required at export. So it's a sibling serializer (`NativeCollectionSerializer` / `CollectionImporter`), not a `CollectionExporter` conformer.
  - **Plumbing (all currently absent).** `UTExportedTypeDeclarations` + `CFBundleDocumentTypes` in `project.yml`; `.onOpenURL` in `FRUSExplorerApp` (double-click / share-to-app); an **Import** affordance in the collections list (reuse the existing XML-sideload `fileImporter` pattern in Settings). macOS `files.user-selected.read-write` entitlement is already present.
  - **Open sub-decisions:** **D9a** — do private research notes/annotations travel (never / opt-in toggle default-off / always)? **D9b** — smart collections on native export (inline the saved-search parameters so the recipient re-runs it — pairs with D8 — vs. export a resolved static snapshot vs. unsupported initially). **D9c** — `.summaryOnly` entries: carry the prompt *text* so the recipient regenerates on-device (graceful fallback to full when AI unavailable) vs. drop to full.
- **D6 — Unify the two Zotero paths.** Today "Zotero RIS" (a format) and "Send to Zotero Library" (a Web-API button) compete in one sheet with different tag/note behavior. Collapse into one "Send to Zotero" with a clear connected-account vs. RIS-fallback story. *(Most involved: the collection-level Web-API path may not exist yet.)*
- **D7 — Non-Zotero citation export.** Add an RIS/BibTeX *file* for other reference managers, reusing the Citation module's existing generators. `RISExporter` already backs the Zotero export; **`BibtexExporter` exists but has no collection consumer** — a small `CollectionExporter` wrapper + a format-picker entry. Cheap–moderate.
- **Destinations polish.** Explicit Print / Mail / Files affordances (iOS share sheet already covers most). Cheap; low priority.
- **D8 — Smart-collection snapshot.** Today membership resolves *at export* from a saved search, so content isn't inspectable in the manager. Option: materialize resolved entries into the manager with a "Refresh from search" action (keep dynamic, snapshot, or offer both). Also unblocks clean native export of smart collections (D9b).
...
| **D9** | Native shareable collection format | No native format (lossy exports only) | `.fruscollection` round-trippable JSON + import (open/share-to-app) — sub-decisions D9a notes-travel, D9b smart-collection handling, D9c summary prompt |

**Status:** Phases 1a–3c + PDF/DOCX structure are **done and merged** (PRs #120–#129). Phase 4 is the last implementation phase; the docs pass follows it.

**Phase 4 decisions (locked 2026-07-01):** build **all four** — D9 (native format), D7 (BibTeX/RIS file), D8 (smart-collection snapshot), D6 (Zotero unification). **D9a = opt-in toggle, default off** ("Include my research notes" switch at export; private annotations stay local unless chosen). Planned sequence (each its own PR merged to v2 before the next): **D7 → D9 → D8 → D6** — cheap/isolated first to de-risk the format pipeline, then the flagship, then the two most-involved. D9b defaults to a resolved static snapshot until D8 lands, then inlines the saved-search parameters; D9c settled at implementation (lean toward carrying the summary-prompt text for on-device regeneration).

*Recommendation when we implement:* the cheap options on D1/D2/D3 are already shipped as part of Phases 1–3. For **Phase 4**, the native format (**D9**) is the flagship — it's the only lossless, editable sharing path and the portability groundwork already exists. Pair it with **D7** (BibTeX/RIS file — near-free given the existing generators). **D8** (smart-collection snapshot) is the natural next expensive item and unblocks D9b. **D6** (Zotero unification) is the most involved and lowest-certainty (depends on a collection-level Web-API path that may not exist) — defer unless prioritized.

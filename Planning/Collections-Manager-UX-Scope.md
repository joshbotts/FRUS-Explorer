...
**Status:** scoped 2026-07-04. **Owner decisions resolved 2026-07-04:** D1 = **"Contents"**; D2 = **faithful `ResearchStripView` clone, no collapse** (macOS strip; iPad native-toolbar promotion; iPhone labeled nav menu); D3 = **pure reporting rows** (all editing → inspector); D4 = **one shared `titleOverride`**; D5 = **empty = all** (mirror highlights; write-on-first-deselect, no data migration; the one intentional export-default change — owner explicitly accepted, flag in release notes). Sits atop the Collections-Authoring program (Phases 1–6) and assumes its inspector (B8), resolver, and `.fruscollection` v2 are in place. Implementation: M1 → M2 → M3, each its own PR.

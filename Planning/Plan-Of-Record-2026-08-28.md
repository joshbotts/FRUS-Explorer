...
| # | Session | Scope | Gate |
|---|---|---|---|
| B-1 | **SESSION 1 SHIPPED 2026-09-01; session 2 outstanding.** W-13 coverage map / systematic-review mode | **Both sources this row named are refuted, as the Tier-E assessment predicted.** `ExportHistoryEntry` records a `documentCount` and no document identities, so it can contribute nothing to "43 of 267"; `ProjectEngagedDocuments.keys` unions visits, notes and collection entries into one flat set, discarding the split the sentence is made of. Session 1 therefore built `DocumentEngagementService` — a second reading of the three real numerators (`ReadingHistoryEntry`, `ResearchNote` + `DocumentHighlight`, collection membership), gathered apart in the `ProjectLeadsService` shape and intersected with the corpus before counting — plus row badges and the coverage line in `CorpusDocumentsView`. Three findings the data forced: opened is a **floor, not a total** when research logging is off (the count still shows, with a caveat); a `DocumentHighlight` carries no `projectId` so it counts through its note, and a standalone one counts only unscoped; and scope is optional throughout, because "have I read this" is worth answering without a project. Refresh hangs on an always-mounted `@Query` observer, never a navigation edge — the change to catch is the reader opening a document from this very list. **Session 2 remains:** the exportable statement in `QueryMethodAppendix` across **all three renderers** (markdown, plainTextLines, csv) plus `preambleLines` and the logging-off caveat, a Project Home tile variant, and both-platform polish. No schema change, as forecast. | none |

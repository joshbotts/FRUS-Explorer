...
| R-5 | **Annotation integrity across an OH correction** — `Volume-Update-Annotation-Integrity-Design.md` | Per-document detection IS achievable: the re-index upsert holds both versions of every document at once, so the changed set is exact. Two hashes are needed, not one — `renderingVersion` excludes footnote bodies by design, and footnote/source-note corrections are a large share of what OH actually changes. **P1 (record the change set at index time, no UI) should ship with the next volume batch**: shipped later it is blind to every correction before it. P2 notifies, P3 repairs. Gate on Q-1, the unmeasured cost of computing the render-space hash at index time. | none for P1 |

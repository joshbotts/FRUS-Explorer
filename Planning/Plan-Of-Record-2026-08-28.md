...
**§7 struck (FROZEN 2026-09-06 — the machine-checked line moved to the live plan of record; this records the state at supersession, and step 11 has since shipped): 0, 1, 2, 3, 4, 6, 7, 8, 9.** That line was machine-checked against the plan itself by
...
| # | Session | Scope | Gate |
|---|---|---|---|
| ~~B-6~~ | **CLOSED 2026-09-04 (PR #1191).** The product decision went to the Mac's own answer rather than the splash — `StatusBarView.activeTask` has shipped a queued-download line all along, so iOS was the odd platform and matching it left O-0-1 untouched. The missing banner state needed no `batch.latest`: it reports a count, as the Mac does, there being no batch to measure. The real cause was that a five-way precedence lived inside `MainTabView.indexingBanner`'s `if`/`else if` chain with no seam — it is now `IndexingInsetState.resolve` and the body switches on it. Two of this row's claims were narrower than written: the *screen* was never blank (the tabs render; the app's account of the pending work was what was missing), and a cloud does still render on a pending Search, since `PendingCloudRule` gates on `indexingBatch != nil`. ~~The blank relaunch window~~ | `.indexingBackdrop` is a suppression verdict **nothing renders**. With a download QUEUED but no batch started, `ContentView` withholds the splash and `MainTabView` never mounts the strip, so a relaunch mid-download shows nothing at all. `CloudSurfaceArbiterTests.relaunchMidDownloadPrefersIndexing` passes while the screen is blank, and now says so in its doc comment. **Not a drive-by**: the fix needs a product decision (a queued-download banner, or let the splash through?) *and* a banner state that does not exist — `IndexingQueueBannerView` needs a `batch.latest`, and in that window there is no batch. | **owner decision** |
...
## 3c. Newly filed — 2026-09-04

| # | Session | Scope | Gate |
|---|---|---|---|
| PV | **Provenance tiers — telling the reader what they may claim** | Wave PV, scoped in full at `Provenance-Tiers-Development-Plan.md`: six sessions (PV-0…PV-5) that label every displayed value with the sources behind it, so a historian can tell "FRUS shows X" from "FRUS joined to NARA suggests X" from "the app's model grouped these". **Ships in the release AFTER build 45** — owner instruction; build 45 is at the store gate. The tier is COMPUTED from the `Env:` inputs the generators already declare, per FIELD not per file, which is what makes it testable rather than a matter of taste. No CloudKit change, no index bump, no new bundled resource. Four owner questions in that plan's §8 gate PV-1. | **build 45 shipped** + §8 Q-1…Q-4 |
...
| R-5 | **P1 SHIPPED 2026-09-03 (PR #1179); P2 SHIPPED 2026-09-03 (PR #1180); P3a SHIPPED 2026-09-03 (PR #1181 — fix it, first half: review stamps, per-highlight confirm/remove from one shared sheet, escalation arm, rebaseline mode, repaint signature); P3b DECIDED 2026-09-03 (design §8.2): Q-3 sync yes; Q-6 G (one mirrored ledger `@Model` — the ninth promotion); Q-7 (b)+(f); Q-8 drafts/(e)/(g) now, (d-cloud) rides the deploy, (b) later; Q-9 (c)+(d); Q-10 (b)+(e)+(f); Q-11 (h)+(f)+(b)+(i). Five PRs in §8.2's order; P3b-1 SHIPPED 2026-09-03 (PR #1182); P3b-2 SHIPPED 2026-09-03 (PR #1183); the NINTH promotion ran the same day (build 44, baseline 260 → 269; attested in PR #1184) — `CD_AnnotationReview.CD_annotationId` alone still awaits, since no build writes a non-nil value and CloudKit creates a field only from one; P3b-3 SHIPPED 2026-09-03 (PR #1185; Q-10 b — the exact seam-aware search and Move Here, with the find-bar complement (f) deferred to P3b-4); P3b-4 SHIPPED 2026-09-03 (PR #1186; Q-7 (b)+(f) — excerpt entries join the review filter through `countsAsAnnotation`, and the sheet runs the shipped export check per quotation. The banner and the row track OPPOSITE texts: `change_kind` is decided by `body_hash`, footnotes EXCLUDED, while the verifier reads the index's `body_text`, footnotes INCLUDED — so an apparatus correction leaves the version equal and changes the haystack. Two draft sentences asserted the reverse and were withdrawn); P3b-5 SHIPPED 2026-09-03 (PR #1187; Q-11 (b)+(i) — the sheet now OPENS what it names, with macOS routing a note to its composer WINDOW rather than nesting a sheet, and the classification parse read LIVE at all three sites that had been using a snapshot nothing ever refreshed: the sentence, both Restore controls, and the mint of a fresh override after a stale restore); **P3b-6 SHIPPED 2026-09-03 (PR #1188 — Q-8 (e) and the carousel-order rider; (b) becomes P3b-7)**; **P3b-7 SHIPPED 2026-09-04 (PR #1189) — R-5's P3b sequence is COMPLETE.** Q-8 (b) split: the *Summarize Again* control ships in the review sheet, and the `sourceContentHash` comparison is REFUSED on THREE measured grounds — `.rebaseline` moves every hash on a Rebuild Index, the vanished mark keeps hashes so equality lies on a removed document, and it is nil wherever a volume has not been re-indexed since P1. **A fourth was claimed twice and withdrawn**: that `content_hash` spans material the summariser never reads, first as "an apparatus-only correction moves it while the summarised text is unchanged", then as "the header, dateline and source note". Both are false — `FRUSASTNode.plainText` recurses `.head`, `.dateline` and `.footnote`, and the source note IS a footnote node, so every element the hash covers is inside the summariser's input. It had been written into this row, the design, `GeneratedSummary`'s doc comment and both manuals before anyone ran the grep** — (e) is five defects rather than the four literals this row records: the oldest-prompt regenerate, SIX unlocalised literals, a `· custom prompt` label that renders for every summary, three silent failure paths, and `loadSummaries`'s `fetchLimit = 20` on a descriptor with no `sortBy`, so which twenty is undefined and a regenerated summary can be absent from the carousel.** Annotation integrity across an OH correction — `Volume-Update-Annotation-Integrity-Design.md` | **Q-1 measured first, and it settled the design**: over the largest post-1960 volume, render-converting every document costs 0.05 s against a 0.50 s parse, so `body_hash` is eager. P1 writes both hashes at index time — `content_hash` over the stored columns, `body_hash` the exact `renderingVersion` highlights carry, pinned against an independent computation — stamps `'body'` / `'apparatus'` / `'vanished'` via one SQL `CASE` upsert, and leaves an identical re-index without a trace. Nine tests through the real `indexVolume`; seven mutations killed, one equivalent (the source note is also in `body_text`). No UI, no `@Model`, no index bump — the table now accumulates truth ahead of the first OH correction, which is the property §6 wanted. **P2 shipped**: `VolumeUpdateReviewSection` in both hubs, a *Changed by an update* row in `ResearchView` over an aggregation widened to `GeneratedSummary` + `ArchiveVisitDocument`, and `DocumentChangeBanner` replacing the twins' two identical banners with one seven-cell truth table; plus the `onVolumeDownloaded` AST-cache fix. **P3a shipped** after a 15-agent reconnaissance that refuted six design premises (the escalation Q-5 claimed, the Unicode-scalar offset unit, `selectedText` as a flat-text substring, the per-annotation grain of the table, a `reviewed_at` writer, and "delete weight elsewhere" as one referent): `reviewed_at` writers at document and volume grain, `DocumentChangeReviewSheet` from both banners and the Research row, Confirm / Remove per highlight, `.rebaseline` for whole-index passes, the widened repaint signature. **P3b** is gated on the owner's answers to design §8.1 Q-6…Q-11 — the re-anchor, summary regeneration, excerpts, non-highlight review state, removed-volume rows, and the remaining modify surfaces. | Q-6…Q-11 in the design |

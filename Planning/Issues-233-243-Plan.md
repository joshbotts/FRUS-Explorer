...
**Empirical results (full corpus, 694 files / 552 series volumes, GENERATED_DATE=2026-07-10):** 2,713,736 `<ref target>` scanned · **652 broken** (unknownPage 392 · unknownAnchor 254 · unknownVolume 3 · malformedTarget 3 · emptyTarget 0) · resolved 2,695,518 · external 9,301 · wholeVolumeRef 6,992 · volumeNotInSnapshot 0. Verified byte-offset + line land exactly on `<ref` in the TEI. Genuine typos flagged: `unknownVolume` = **frus1873p2v2** (→v3), **frus1888**; `malformedTarget` = bare `pg_1602`/`pg_1743` page anchors written without `#`. Outputs at **`Planning/cross-ref-validation/`**: `broken-refs-report.{csv,json}` (OH-submittable) + `broken-refs-index.json` (**candidate** bundled index, **73 KB full-detail** — ~20× under the 1.5 MB threshold, so ship full detail).

**Session 7 inputs from this run:** (a) bundled-index shape = **full-detail, composite `(sv,sd,t)` key** (empty-string `sd` sentinel for front/back-matter refs — 621/652 broken refs are outside document divs); `rv`/`ra` ride along for the explanation sheet. (b) Move `broken-refs-index.json` from `Planning/` → `Resources/` and add to app-target resource lists (Session 7 step 1). (c) **Non-shippable** = 1,273 refs → 58 corpus volumes outside the 552-volume manifest (the 1981-88 / 1989-92 / mSupp series present on disk but not shipped) — resolve in-corpus, un-navigable in-app; a Session-7 *policy* input (grey out vs. leave live), **not** broken. (d) No `currentDateIndexVersion` bump this session; Session 7's `is_broken` UPDATE pass still substitutes per the index rule.

...
| W-8 | **Pre-1910 Phase-3 tail harvest** — **CONSULAR TAIL DELIVERED 2026-08-27, OFFLINE** | The three consular series shipped without the API key the row assumed: measured against the bulk record-group shard, all three are single chronological runs whose FILE UNITS are the volumes, complete against NARA's own per-series `fileUnitCount` — so `CONSULAR_TAIL_FROM_HARVEST=1` builds Consular Instructions (604019, 7 vols), Notes to Foreign Consuls (1076611, 4), Notes from Foreign Consuls (1076629, 11) into `central-files-index.json` from `rg_59.json`, with the classifier + date-only resolution path in Source Explorer on both platforms. **Remaining in this row**: Domestic & Miscellaneous Letters and Special Agents (offline-servable the same way per the 2026-08-27 assessment — Misc Letters is 14/98 digitized — but their CLASSIFIER cues are undesigned: what marks a Department letter as domestic?), and the two June leftovers (enclosure dual-home rendering; live UI walkthrough). The keyed loop now includes the new categories, so any future keyed run refreshes them live. |

...
| CloudKit Production promotion for the W-4+W-5 fields (one Dashboard step, batched) — **the block is now complete: 41 identifiers** (Archive Visits Phase 2's 32 + W-4's `DocumentClassificationOverride` record type at 7 + W-5's two `SavedSearch` fields), all in `identifiersAwaitingDeploy` with the checklist in that list's comment. Owner steps before deploying: exercise each new type/field once on a Development build signed into iCloud (create a plan / seed a document / tier a target; reclassify one document; run one saved search), then Dashboard → Deploy, clear the list, restate the baseline count + digest, bump `deployedThroughBuild`/`deployedOn`. Until then, all three features' records stay device-local-in-effect: Production rejects them and the app says so at launch and in Settings ▸ Data & Recovery ▸ iCloud Schema | Tier B |

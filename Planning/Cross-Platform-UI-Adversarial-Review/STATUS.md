...
| **P-8** (iPhone) | Cross-Reference Analytics is "the worst-crowding view" | **Its own evidence is stale.** That code comment was falsified by #209 thirty minutes after it was committed; the surface is now the *least* crowded in the family, one glyph in an empty bar. The surface that genuinely truncates (`Col… Net… Flo… You…`) is Archival Analytics' mode picker, which P-8 does not name — fixed in #908. **The comment itself stayed in the source until #918**, which is how a stale comment became a review finding. |
...
|---|---|---|
| **F-2** — iPad width discipline, given that capping a root `List` breaks its native style | **Two-pane at regular width** — SHIPPED — Browse in #914, Research in #915, as §7.1 after two shapes were refuted by measurement (#912 nested stack, #913 nested split), and **corrected at the document level in #921**: the Research rail is a third column, so the reader was left 451.5pt on a 13-inch iPad (design §8). **Complete.** | Browse and Research gain a list + detail layout following `CollectionEditorView.iPadCollectionLayout` (`:734`, reached via `if sizeClass == .regular` at `:518`). Must honour the #238 rule: a plain `HStack` of two panes, **never** a nested `NavigationSplitView`. This is the largest of the four options and was chosen over my recommendation of capping row content — so the row-content cap is NOT a fallback to drift into; if the two-pane shape proves wrong, that is a new decision. |

...
Last updated after PR #914. Shipped: Wave 1 (CW-1…CW-5), Wave 2 (CW-6…CW-8a), and Wave 3 so far (CW-11a, CW-10a/b, plus #901's by-catch).
...
|---|---|---|
| **F-2** — iPad width discipline, given that capping a root `List` breaks its native style | **Two-pane at regular width** — SHIPPED in #914 as §7.1, after two shapes were refuted by measurement (#912 nested stack, #913 nested split). Research is untouched and remains open. | Browse and Research gain a list + detail layout following `CollectionEditorView.iPadCollectionLayout` (`:734`, reached via `if sizeClass == .regular` at `:518`). Must honour the #238 rule: a plain `HStack` of two panes, **never** a nested `NavigationSplitView`. This is the largest of the four options and was chosen over my recommendation of capping row content — so the row-content cap is NOT a fallback to drift into; if the two-pane shape proves wrong, that is a new decision. |

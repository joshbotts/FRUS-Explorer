# #234 early-era people, reframed — ASSESSMENT DELTA

**Status: ASSESSMENT, 2026-09-13. Not a plan of record, not a schedule, and nothing in it is queued. Owner decisions: PENDING — the revised list is §6.**

This delta re-reads `Planning/234-Early-Era-People-Feasibility-Assessment-2026-09-12.md` (PR #1290) and two later in-session answers under a reframed #234. Three reader agents and five offline measurements produced it. Each measurement was independently reproduced, and a synthesis, a critic, a corrected rewrite, an independent check of that rewrite, and 14 residual fixes by hand followed. The first draft is `reframe/synthesis/reframe-delta.md`, the critique `reframe/critique/critique.json`, and the change log `reframe/synthesis/reframe-delta-v2-changelog.md`. Evidence paths are relative to `Planning/early-era-people/reframe-2026-09-13/`, whose README lists what was not copied.
...
**Evidence keys.** Paths are relative to the evidence root, `Planning/early-era-people/reframe-2026-09-13/`. Files over 1 MB, build caches and binaries were not copied; the root's README lists them, and each is regenerable from its script against the stores.
...
| 6 | M2a keyed | 64 of 72 keyed; keying the other 8 optional, as a held-out set | **tightens** | The 8 unkeyed documents (3 / 3 / 2 in the three later bands; MEASURED, FA §0) are the only staged documents whose text and spans sit in no committed file. #1290's `feasibility-2026-09-12/verify-method/stores.json` does name all 8 by staged filename, with size, seed count and hash only (MEASURED, checked against the repository before this file was committed). They are too few to score a verifier, so they should be spent only as a held-out check. | Protect them from prompt design and pre-fill |

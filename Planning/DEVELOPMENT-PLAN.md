...
- Design defaults chosen (the open questions): each macOS document window carries its **own ResearchStrip** (full workspace, not a lean viewer); macOS **search-opens-in-window is deferred** (kept iPad-only this round — the macOS search window already routes documents into the main window).

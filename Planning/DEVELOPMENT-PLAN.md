...
## Session 2026-08-27K — the sitting's evidence improved: prose-first snippets

Owner request before starting the sitting: the 240-char body prefix spent most of its
budget re-showing the header and source note the row already states. EvalSnippet now
strips the body's own front matter — matched against the DOCUMENT'S OWN stored fields
(header / document_sources.raw_text / dateline), in any order, because the layouts
differ by era (1943: citation → header → dateline → serial; 1961: header → Source: …) —
plus the two pieces no field carries (a short [Received …] bracket, a despatch/telegram
serial up to seven digits), with an all-boilerplate fallback to the plain prefix. 300
chars at a word boundary. Two blemishes found by reading the first regenerated output:
six-digit modern telegram serials survived a five-digit cap, and every truncated row
wore a double ellipsis (builder + truncator each appending one). The report header now
also carries the corrected judging rubric (the would-open test with near-misses scoring
1; known-item hits computed separately; blank excluded from the denominator) so the
sitting's rules live where its results do. Rankings untouched — verdicts.csv and
stats.json regenerated byte-identical, which is itself the confirmation that only
display changed. Verdicts remain blank per the owner's reset request.

## Session 2026-08-28A — the sitting judged: the verdict is a register split

The owner filled all 435 verdict rows (173 relevant / 144 not / 118 blank, zero
malformed; one blank-vs-judged pair resolved by carry-over, disclosed). Computed and
recorded at Planning/semantic-vectors/eval-2026-08-27/VERDICT.md:

Semantic P 0.65 / MRR 0.771 against lexical P 0.40 / MRR 0.418 — and the aggregate is
the least informative number. Research questions: 0.65 vs 0.07, with lexical returning
ZERO rows on four queries (implicit AND). Terms of art: the mirror image, 0.10 vs 1.00
on both "trust but verify" and persona non grata — embeddings dissolve the phrase that
IS the discriminator. Plain keywords: tied (0.77/0.79). The Olney note and the Mao
memcon were found semantically (the note with its whole surrounding correspondence at
ranks 1–6); the blood telegram beat both routes — the nickname query class scored zero
across the board, and Trueblood's telegrams are the cleanest exhibit of the surname-
collision hazard. The wrong-premise probe worked (semantic corrected past "sinking",
0.60 vs 0.00); the null control half-worked (the judge honestly found three would-open
rows — the next set needs a harder null) but its real finding stands: semantic junk
arrives wearing confident scores.

Decisions recorded: W-9 proceeds strengthened (the P 0.65 / MRR 0.77 bar for step 1,
the instrument for step 3, direct evidence for step 4; prompt settled = query template);
a shipped search surface should route by register, cheapest integration = semantic
...
(b)+(f); Q-8 drafts excluded, (e) and (g) now, (d-cloud) riding Q-6's deploy, (b) later, (c)
refused; Q-9 (c)+(d); Q-10 (b)+(e)+(f); Q-11 (h)+(f)+(b)+(i), the purge refused. Recorded in the
design's §8.2 with the five-PR sequence. This is the first of the five: everything that needs no
deploy and no design.

**What shipped.**
- **Q-9 (c)** — `unreviewedDocumentRevisions()` excludes volumes with no `document_cache` row, at
  VOLUME grain (a vanished document has no cache row by design, so a document-grain join would drop
  the design's worst case). A removed volume's rows are kept and return intact on re-index, pending
  reviews included; a document dropped while the volume was gone reads as vanished on return.
- **Q-9 (d)** — `clearDocumentRevisions()`, called from Erase Everything and nowhere else. Not the
  rebuild wipe (`indexAllVolumes` rebaselines after it to keep stamps) and not `resetLocalData`,
  which "Reset This Device" also calls while promising the annotations return.
- **Q-11 (h)** — a Research row whose revision is `'vanished'` opens the review sheet instead of a
  document that lands on "Failed to Load".
- **Q-11 (f)** — the vanished-row delete also removes the `document_sources` row. It was
  `INSERT OR REPLACE` per surviving document with no per-volume clear, so a live archive-visit plan
  kept deriving a target from a document that no longer existed.
- **Q-8 drafts** — headnote drafts leave `annotatedKeys` and the row's summary count through one
  pure counter; the hub inherits.
- **Q-8 (g)** — both FTS5 push loops (boot reconcile, post-download) send the newest non-draft
  summary per document through `GeneratedSummary.newestNonDraftPerDocument`, instead of every row
  in fetch order with last-write-wins.
- **Q-7 (g)** — the export sheet upgrades a verifier `documentNotIndexed` to `documentVanished`
  when the revision row says so, and the report says "cite a document that an update removed from
...
## Session 2026-09-03f — the ninth CloudKit promotion, attested (PR #1184)

The owner exercised both additions on a Development build and deployed. Closing the loop is the
gate's step 5, and it is the step that must not be done early: restating the baseline is the claim
that Production matches this build.

**Nine of ten, and the tenth is left honest.** `CD_AnnotationReview.CD_annotationId` is nil in
every record this build writes — every writer records a `document`-grain row — and CloudKit creates
a field from the first record that carries a value, so the field was never created in Development
and cannot have been promoted. It stays in `identifiersAwaitingDeploy` rather than being swept into
the attestation, which is what that list is for: an honest "not deployed yet" instead of a claim
nobody checked. `CloudKitSchemaInventoryTests` cannot catch this either way, so P3b-4 and P3b-5 —
the phases that give the reserved kinds their writers — carry the obligation to exercise and
promote it before shipping.

`deployedIdentifierCount` 260 → 269 with the digest the suite printed, `deployedThroughBuild`
43 → 44, `deployedOn` → 2026-09-03. The recipe the owner followed is worth recording because one
step was not obvious: the installed build predated `document_revisions`, so the table was created
empty on first launch and the first re-index of a volume writes only a baseline. A single
edit-then-reindex stamps nothing, with no error to explain it. Two passes are needed — baseline,
...
## Session 2026-09-03h — R-5 P3b-4: a quotation is research too (PR #PRNUM)

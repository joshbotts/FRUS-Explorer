...
| **Q-5** | Should an update be offered at all while unreviewed changes are pending from the last one? | A reader who updates twice before looking gets two change sets over one baseline. The revision table handles it; the copy has to. **P2 note:** the `CASE` upsert re-stamps `change_kind` on every change, so a `'body'` change followed by an `'apparatus'` one reads as *apparatus* although the highlight space moved since the reader last looked. This is why `DocumentChangeBanner` keeps BOTH inputs — the recorded kind and each highlight's own `renderingVersion` against the document's — and says "some highlights may be misaligned" whenever the second disagrees, whatever the first says. P3's review action should stamp `reviewed_at` and leave the kind's escalation (body wins over apparatus until reviewed) to the upsert. |

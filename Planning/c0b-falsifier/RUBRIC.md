# C-2 long-session re-run — PRE-REGISTERED scoring rubric

Written 2026-08-31 BEFORE any C-2 run was launched and before any output was read. Frozen.

**This is C-0's rubric with items A1-A5 and every other item UNCHANGED, so the two experiments
compare directly, plus ONE ADDED ITEM, A6.** C-0's §6 asked for A5 to be *sharpened*; sharpening it
in place would have broken comparability with the run that motivated the sharpening, so the loose
item stays and the strict one is added beside it. A6 is the item that discriminated in C-0 and had
to be recovered post-hoc there (block arm 4/4 and 14 distinct NAIDs; control arm 0/4).

## What C-2 changes about the DESIGN

C-0 measured whether the rules WORK. Every one of its runs was 30-60 tool calls in fresh context, so
it could say nothing about whether the rules SURVIVE — which is the one thing a re-presented tool
catalogue is claimed to do better than a pasted block.

C-2 therefore runs a LONG session. The house-rules block, where present, appears once at the top.
Three substantial, deliberately NON-ARCHIVAL tasks run first. The terminal task is then **one of
C-0's two questions, worded identically, with C-0's identical deliverable and tool-call budget** —
so the memo scored here and the memo scored in C-0 differ in exactly one respect: how much work sits
between the rules and the task.

The terminal task is withheld from the opening prompt. It sits in a file the runner is told not to
open until tasks 1-3 are done, so it enters context as a tool result deep into the session rather
than adjacent to the rules. Without that, an agent could plan its archival work at turn 1 with the
rules fresh, which is the condition C-0 already measured.

Both arms are run again at long-session length, because a drop in the BLOCK arm would otherwise be
unattributable: it could be the rules decaying, or it could be that everything gets worse in a long
session. The no-rules long arm separates those.

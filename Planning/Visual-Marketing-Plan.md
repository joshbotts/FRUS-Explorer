...
|---|---|---|---|---|
| Launch splash cloud + shimmer | pre-bundled | continuous | **Drift (Canvas)** since M-4; the launch storyboard now hands over from a baked plate of the same words (`LaunchCloud`, 2026-09-20) | `LaunchSplashView.swift` (`drift: true`) |
...
11. ~~**Do not put any image beyond the existing dim tile on the iOS launch storyboard.** It runs no
    ...
    **Both images now ship, and the tile is the app icon.** `LaunchCloud` is the plate #1346
    generated: packed by the app's own `WordCloudLayout` from `cloud-vectors-core.json` on the
    `.concepts` lens the splash opens on, generated by `FRUSExplorerTests/LaunchArtworkTests`
    behind `RENDER_LAUNCH_ARTWORK_DIR`, per idiom (720 x 400 plate points kept clear for a phone
    against 460 x 250 for an iPad — each the measured worst case over that idiom's canvases and
    each pinned by a test that failed on its first run) and per appearance, ink baked and
    ceilinged at `FRUSTheme.cloudDimIndexingStrip` — shipped as PNG renditions at or under
    2400 px on an edge, because the 4200 px raster `actool` makes of the PDF at 3x drew the whole
    launch screen black on both devices. `LaunchAppTile` is no longer a generic blue
    document-and-magnifier tile that appeared nowhere else in the product: it is the 1024 pt icon
    artwork clipped to the home screen's continuous corner at 1x/2x/3x, with a `mac` idiom
    carrying the macOS icon. `LaunchIdentityBlock` draws the same block on the splash and on
    onboarding's welcome step, so the fresh-install sequence keeps one icon in one place while
    the cloud arrives, the shimmer goes and the dock rises.

    **The "unexplained asset lookup" #1346 reverted the plate over was the DEVICE, not the asset.**
    A launch screen carrying seven new assets — the plate as PDF and PNG, a byte-identical copy of
    `LaunchAppTile` under a new name, a 400 pt PDF with embedded fonts, the same with the text as
    outlines, a 300 pt PNG crop and the app icon at three scales — drew none of them after
    uninstall + reinstall on the iPad #1346 measured on, while drawing `LaunchAppTile` in the same
    frame; after `simctl shutdown` + `boot` + reinstall, and on a never-used simulator, all seven
    drew. The launch-screen renderer keeps the app's asset catalog in memory per bundle identifier
    across reinstalls, so every configuration #1346 "eliminated" was tested against a stale
    catalog, and the template-ink route it recorded as not working is unproven rather than
    refuted. **Reboot the simulator after changing a launch-screen image**; the storyboard's own
    header says so.

...
**Status:** COMPLETE 2026-07-04 — all five phases delivered (Phase 1 extraction fix; Phase 2 SourceNoteKit grammar + eval harness; Phase 3 volume_sources keying/inheritance/matcher; Phase 4 bundled collection authority + app surfaces; Phase 5 three-state neighbors, classification chips, S6 macOS neighbors window, program docs pass). Decision resolutions: **S1** = yes (classification stored at parse time, surfaced in Phase 5 as chips in Source Explorer, the reading view's source footnote, and search rows); **S2** = SPM tool (`SourceNoteEvalGenerator`, diffable `eval-report.txt`); **S3** = prefix-match (cheap path; period-aware segmenting not needed for front-matter leaves); **S4** = two levels (collection + sub-series in `collection-authority.json`); **S5** = local (In-Your-Library counts always computed from the user's own index; series-wide volume lists bundled); **S6** = window (Archival Neighbors is a value-based `WindowGroup` on macOS — one restorable window per distinct `ArchivalNeighborsRequest`, all macOS presenters converted; iOS keeps sheets). Delivered outcomes vs targets: modern-era (1955+) extraction restored (~77k documents); eval unrecognized rates 2.6–6.1% for most 1955+ eras (15.2% for 1961–63; pre-1952 eras remain above the aspirational targets — dominated by pre-decimal-era citation styles out of scope for this program; see `eval-report.txt`); authority: 4,431 records, 100% of lot-keyed front-matter items landing, 939 NAIDs offline.

...
/// (Analytics SA, series chart refinements): the coverage-year → target mapping
/// (none / 15 / 20 / 30) at every breakpoint, and the domain-clamped step-line
/// point generation (no point before 1961; the editable range is respected). No
/// SwiftUI is instantiated.
///
/// Version history:
...
struct PublicationLagTargetTests {
    ...
    func targetValuesAtBreakpoints() {
        ...
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1960) == nil)
        // 1961 ≤ year < 1972: 15 years (1961 directive).
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1961) == 15)
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1971) == 15)
        // 1972 ≤ year < 1985: 20 years (1972 directive).
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1972) == 20)
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1984) == 20)
        // year ≥ 1985: 30 years (1985 directive, 1991 statute).
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1985) == 30)
        #expect(PublicationLagTarget.targetYears(forCoverageYear: 1993) == 30)
    }
    ...
    func stepPointsFullDomain() {
        ...
        #expect(points.map { [$0.coverageYear, $0.targetYears] }
    ...
    func stepPointsNoPointBefore1961() {
        let points = PublicationLagTarget.stepPoints(in: 1861...1993)
        #expect(points.allSatisfy { $0.coverageYear >= 1961 })
        // The first point sits exactly at the first directive year.
        #expect(points.first?.coverageYear == 1961)
    }
    ...
    func stepPointsNarrowedRange() {
        ...
        #expect(clampedStart.first?.coverageYear == 1961)
    ...
    func stepPointsSingleYear() {
        ...
        #expect(points.map { [$0.coverageYear, $0.targetYears] } == [[1990, 30]])
    }
}

...
// MARK: - Handoff (UI review F-28)

/// Pins the map's Handoff payload and, above all, its registration.
@Suite("Semantic map — handoff")
@MainActor
struct SemanticMapHandoffTests {

    @Test("A request survives the userInfo round trip")
    func requestRoundTrips() throws {
        let request = SemanticMapRequest(volumeIDs: ["frus1969v12", "frus1969v13"],
                                         scopeLabel: "Nixon", lensRawValue: "era")
        let restored = try #require(SemanticMapRequest.from(userInfo: request.userInfo))
        #expect(restored == request)
    }

    @Test("An unscoped map round-trips as unscoped, not as an empty scope")
    func unscopedRoundTrips() throws {
        // The keys are omitted rather than encoded as null, so this also pins that an absent key
        // reads as "whole series" — the difference between showing the corpus and showing nothing.
        let request = SemanticMapRequest(volumeIDs: nil, scopeLabel: nil, lensRawValue: "cluster")
        #expect(request.userInfo.count == 1)
        let restored = try #require(SemanticMapRequest.from(userInfo: request.userInfo))
        #expect(restored.volumeIDs == nil)
        #expect(restored.scopeLabel == nil)
    }

    @Test("A payload with no lens is refused")
    func lessPayloadIsRefused() {
        // The one field with no sensible default. Everything else absent means "whole series".
        #expect(SemanticMapRequest.from(userInfo: ["scopeLabel": "Nixon"]) == nil)
        #expect(SemanticMapRequest.from(userInfo: nil) == nil)
    }

    @Test("An unknown lens is kept, not rejected")
    func unknownLensSurvives() throws {
        // An older build receiving a lens it has no case for should still open the scoped map;
        // the view ignores a lens it cannot resolve rather than dropping the whole continuation.
    ...
    ///
    /// Handoff routes an activity to an app only if its type string is registered in that target's
    /// `Info.plist` under `NSUserActivityTypes`. Publishing and continuing can both be perfectly
    /// wired and the feature still does nothing. Nothing in this repo pinned that registration —
    /// `grep NSUserActivityTypes FRUSExplorerTests/` was empty before this — and the plists are
    /// **generated by XcodeGen from `project.yml`**, so editing a plist by hand is reverted by the
    /// next `xcodegen generate`. This reads the built app's own plist, so it fails whether the
    /// declaration was never added or was regenerated away.
    @Test("Every activity type the app publishes is registered for Handoff")
    func activityTypesAreRegistered() throws {
        let declared = try #require(
            Bundle.main.object(forInfoDictionaryKey: "NSUserActivityTypes") as? [String])
        #expect(declared.contains(AppActivityTypes.document))
        #expect(declared.contains(AppActivityTypes.semanticMap),

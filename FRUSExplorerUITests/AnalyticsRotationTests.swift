...
final class AnalyticsRotationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        // #498 bisect: forward any FRUS498_* switch set on the test runner into the app under test,
        // so candidates can be isolated with `xcodebuild ... TEST_RUNNER_FRUS498_X=1` against a
        // SINGLE build instead of rebuilding per candidate.
        var forwarded: [String] = []
        for (key, value) in ProcessInfo.processInfo.environment where key.hasPrefix("FRUS498_") {
            app.launchEnvironment[key] = value
            forwarded.append("\(key)=\(value)")
        }
        // Logged so a bisect run can PROVE the switches reached the app. A silently-empty forward
        // would make every "still hangs" result a false negative.
        XCTContext.runActivity(named: "FRUS498 switches forwarded: \(forwarded.sorted().joined(separator: ",")) (count \(forwarded.count))") { _ in }
        app.launch()
    }
    ...
    func testRotateWithMultipleTermsAndKeyboardUp() throws {
        ...
        let chipsElided = ProcessInfo.processInfo.environment["FRUS498_NO_TERM_CHIPS"] == "1"
        for term in ["Moscow", "Paris"] {
            addField.tap()
            addField.typeText("\(term)\n")
            if chipsElided {
                // No chip to observe; give the commit + reload a moment to settle instead.
    ...
    func testRotateWithTwoTerms() throws {
        ...
        field.tap()
        field.typeText("Berlin\n")
        // With FRUS498_STATIC_PLACEHOLDER the placeholder never flips, so the field cannot be
        // located by the "Add a term…" label — fall back to the only text field on screen.
        let staticPlaceholder = ProcessInfo.processInfo.environment["FRUS498_STATIC_PLACEHOLDER"] == "1"
        let addField = staticPlaceholder ? app.textFields.firstMatch : app.textFields["Add a term…"]
    ...
    private func rotateRoundTrip() {
        // #498 bisect: optionally let everything settle before rotating. If the hang disappears
        // with a settle delay, the trigger is a race — async `reloadData()` state landing while the
        // rotation's graph transaction is open — not a static view-structure cycle.
        if let seconds = ProcessInfo.processInfo.environment["FRUS498_SETTLE_SECONDS"],

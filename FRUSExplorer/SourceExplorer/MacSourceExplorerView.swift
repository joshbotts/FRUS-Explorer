...
///   1.7 — 2026-09-13: the pre-1906 box runs through the shared `CentralFilesClassifier.evaluate` —
///          a header and dateline the route did not pass are read from the index, a letter to the
///          U.S. chief of mission reads as a "Likely" instruction badged with the register that
...
struct MacSourceExplorerView: View {
    ...
    /// What the pre-1906 box knows: loading, not checked (and why), no match, not applicable, or the
    /// homes found (Phase 2).
            ...
            VStack(alignment: .leading, spacing: 14) {
                GroupBox(String(localized: "source.explorer.rawNote.header",
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(hasSourceNote ? rawSourceNote : noSourceNoteBody)
                            .font(.callout)
                ...
                // #829a: the third body of archival evidence, beside the source note above.
                if !unprintedPointers.isEmpty {
                    unprintedBox
                }
            }
    ...
    // MARK: - Unprinted Material (#829a, ported)
    ...
    private func naraBox(for parsed: ParsedSourceNote) -> some View {
        ...
        case .centralFiles(let rg, let fileId):
            ...
            if ParisPeaceRecords.applies(recordGroup: rg) {
                ...
                GroupBox(header) {
                    VStack(alignment: .leading, spacing: 10) {
                        if let fileId, let year = effectiveYear, (1906...1910).contains(year) {
    ...
    /// iOS twin, which keeps the same alias, so the two cannot come to disagree about a row's identity.
    typealias CountrySeriesResolution = CentralFilesResolution

    /// The homes the pre-1906 box lists — empty unless it resolved.
    private var countryResolutions: [CountrySeriesResolution] { countrySeriesOutcome.homes }

    /// The year every year-dependent box reads: the index-hydrated one, else the host's.
    ///
    /// `loadIdentity` deliberately keeps reading the host's `documentYear`: a key that read the
    /// hydrated year would change the moment hydration landed and reload the window.
    private var effectiveYear: Int? { documentContext?.year ?? documentYear }

    /// Classifies a pre-1906 document (no source note) through the shared
    /// `CentralFilesClassifier.evaluate` and stores what it found. Mirrors the iOS twin.
    ///
    ...
    /// is written only when this load was not cancelled: this window stays alive across documents, and
    /// a key change mid-evaluation would otherwise let one document's answer land on the next.
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                if case .resolved(_, let serialLabel) = countrySeriesOutcome,
                   let serial = documentContext?.despatchSerial {
                    ...
                    // NARA rows it would read as a resolution it cannot make. Its label follows the
                    // lead home's direction (`CentralFilesSerialLabel`), exactly as on iOS.
                    VStack(alignment: .leading, spacing: 2) {
                        Label {
                ...
                ForEach(countryResolutions) { resolution in
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        HStack(spacing: 6) {
                            ...
                            // bundled NARA artifact answered — `CentralFilesClassifier.evaluate` returns
                            // no home otherwise. The chip sits beside the confidence capsule, where
                            ...
                            if resolution.chiefOfMission != nil {
                                // D1: a row the addressee rule promoted to "Likely" rests on the Office of
                                // the Historian's register as well — it named the chief of mission.
    ...
    /// Shown for a document with no source note whenever the pre-1906 box did not resolve to a roll:
    /// while it checks, when it could not check (saying why), when it checked and nothing matched, and
    /// for a document from 1906 on, where it does not apply. Only "nothing matched" says the filing
    /// couldn't be predicted from the dateline and chapter, because it is the one state in which that
    /// was tried. Every state adds the era's likely series when the year is known. Mirrors the iOS
    /// twin's `noSourceNoteSection`.
    @ViewBuilder
    ...
    /// The left column's sentence for a document with no source note.
    ///
    /// `noNote.body` points at "the resolution on the right", which is only honest while the right
    /// column is checking, has resolved, or found nothing. When the check did not run, or does not
    /// apply, the left column says that instead.
    private var noSourceNoteBody: String {
        switch countrySeriesOutcome {
        case .notChecked:
            return String(localized: "source.explorer.noNote.body.notChecked",

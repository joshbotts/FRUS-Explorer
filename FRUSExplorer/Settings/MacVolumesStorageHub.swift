...
/// Count-agreeing copy fragments for the hub (S-2b).
///
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAllVolumesSheet) {
            MacAllVolumesSheet(
                ...
                onRemove: { volumeId in await removeVolume(volumeId) }
            )
            .environment(appState)
        }
...
/// The user multi-selects, sees a running estimate of recoverable space, and confirms. A VACUUM
/// runs after all removals complete to shrink the index file immediately.
///
...
private struct MacManageStorageSheet: View {
    ...
    /// The storage measurement the candidate list is drawn from.
    let report: StorageReport
    /// Volumes carrying user data — excluded from the candidate list entirely.
    let protectedVolumeIds: Set<String>
    /// Most recent open per volume, which sets the removal ordering.
    let lastOpenedByVolumeId: [String: Date]
    /// The app state used to remove volumes and refresh dependent stores.
    let appState: AppState
    /// Called after removals complete so the host can re-measure.
        ...
        candidates
            ...
            .reduce(0) { total, c in
                total + c.entry.volumeFileBytes
                    + Int(Double(c.entry.volumeFileBytes) * StorageReport.indexOverheadFactor)
            }
    }
                    ...
                    if selected.isEmpty {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text(String(localized: "settings.hub.freeUp.recovery",
                                        defaultValue: "~\(ByteCountFormatter.string(fromByteCount: Int64(estimatedRecovery), countStyle: .file)) estimated recovery"))
                                .font(.callout)
                                .fontWeight(.medium)
    ...
    private func candidateRow(_ candidate: (entry: VolumeStorageEntry, lastOpened: Date?)) -> some View {
        ...
        let totalEstimate = candidate.entry.volumeFileBytes
            + Int(Double(candidate.entry.volumeFileBytes) * StorageReport.indexOverheadFactor)
    ...
    private func performRemoval() async {
        ...
        for volumeId in selected {
            do {
                ...
                removalError = String(
                    localized: "settings.hub.freeUp.error",
                    defaultValue: "Failed to remove \(volumeId): \(error.localizedDescription)")
                // Whatever did come out is already gone from the aux tables — reopen the
                // read-only stores before bailing out (#275).
                appState.refreshReadOnlyStores()
                isRemoving = false
                await onComplete()
                return
        ...
        // Removals deleted aux-table rows the boot read-only connections still hold open, so
        // analytics would keep counting the removed volumes until relaunch (#275). The sheet had
        // never done this; the single-volume path in the old Storage pane always had.
        appState.refreshReadOnlyStores()

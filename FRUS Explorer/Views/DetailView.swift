...
struct DetailView: View {
    ...
    private func findDivision(nodeID: String, volume: LoadedVolume) -> Division? {
        ...
        return search(nodeID: nodeID, in: vol.text.body.divisions, volumeID: volume.id)
            ?? search(nodeID: nodeID, in: vol.text.front?.divisions ?? [], volumeID: volume.id)
            ?? search(nodeID: nodeID, in: vol.text.back?.divisions  ?? [], volumeID: volume.id)
    }

    private func search(nodeID: String, in divs: [Division], volumeID: String) -> Division? {
        for div in divs {
            if volumeID + "__" + (div.id ?? "") == nodeID { return div }
            if let found = search(nodeID: nodeID, in: div.children, volumeID: volumeID) { return found }
        }
        return nil
    }
}
...
enum FRUSRenderer {
    ...
    private static func collectInlineNotes(from content: [InlineContent], into result: inout [Note]) {
        for item in content {
            switch item {
            case .note(let n):
                // Inline notes in FRUS are always footnotes
                result.append(n)
            case .hi(let h):
        ...
        case .hi(let h):
            ...
            if rend.contains("italic") {
                ...
                var sc = AttributedString(a.characters.map { c in
                    c.isLowercase ? Character(c.uppercased().first ?? c) : c

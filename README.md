...
- Full-text search and filtering across the FRUS corpus (FTS5, English stemming, BM25 ranking), with a **volume/subseries scope** (platform-appropriate pickers; "Search this volume" handoff from the browser), grouped/Boolean query support, a front-matter inclusion toggle, and **user-tag filter chips** (on iOS/iPadOS these refresh live as you create, rename, or delete tags anywhere in the app). On iOS/iPadOS the main results list also gains an **adjustable snippet length** (1–10 lines) and an optional **Checklist review mode** that hides each result as you open it or mark it reviewed, with a progress banner and an all-reviewed empty state. Snippet length is set by a global default in Settings, honored by the cross-platform Collections add-documents sheet (below) and, on iOS/iPadOS, the main search list, each with its own per-surface override

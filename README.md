...
- **Reading history**: every document visited and every search executed is recorded (subject to Settings → Research → **Log Research Sessions**), and browsable on **both platforms** through one shared History screen — macOS in the **Complete History** window, iOS/iPadOS pushed from the Research tab. Scope it to All Projects / Not in a Project / one project, filter it by free text, re-open a document or re-run a search with a single click, and delete individual entries. Loads 500 rows per section with a **Show More** control and an honest "Showing N of M" header rather than holding the whole (unpruned) trail in memory. macOS additionally keeps its **History** menu (last ten of each, with quick re-open/re-run)
- **Embedded in-app browser** for Markdown links throughout onboarding, About, and education content, plus a standalone **FRUS Research Guide** (Settings → About → Resources on iOS; dedicated window + Help-menu command on macOS) with contextual deep-links from Source Explorer and NARA Catalog Lookup
...
- **Four analytics surfaces** — Corpus term-frequency, **Person**, **Cross-Reference**, and offline **Series** dashboards — cover the corpus from complementary angles (described in the four bullets that follow); Corpus, Person, and Cross-Reference share a **subseries/volume scope** and **year-range** bar so any of them can be narrowed to a project-specific slice (Corpus and Cross-Reference Analytics add an **Administration** preset menu that sets the document-year range to a president's term in one tap); **Person and Cross-Reference Analytics group their charts into collapsible sections** with each chart's controls in its own section, and on iPhone Corpus Analytics folds its secondary toolbar controls into an **Options** menu to fit the compact width; Corpus, Person, and Cross-Reference Analytics also offer **research-grade export** — an **Export** menu (view-level in the Corpus Analytics toolbar, or inside its iPhone **Options** menu; per-section in Person and Cross-Reference Analytics) writes **Chart data (CSV)** with a `#`-commented provenance preamble (figure, terms, grouping, scope, year range, value mode, app version and export date, then method, caveats, and corpus attribution) plus **Figure (PNG)** at 2× and vector **Figure (PDF)** on a 1,200 pt white plate wherever a figure is honest (Corpus offers figures on the time axes only; ranked-table sections such as Landmark Documents are CSV-only, and the Person network graph has no export), delivered through an `NSSavePanel` on macOS or a share sheet on iOS as `FRUS-Analytics-<chart>-YYYY-MM-DD.<ext>`, with a failed export reported in an alert
...
- **Collocates** (Search, both platforms): what appears near your query. Collects the words within ±5–50 of every match across the **whole retained result set** (not the page — a page cannot clear the measure's floor), counts them with the app's own NLTagger lemmatiser, and ranks them by log-likelihood keyness against the bundled corpus reference. Bounded by a window-scaled token budget rather than a document count, with the bound stated on screen; rows carry the effect size, both raw counts, and an explicit *unpriced* marker where the reference is too shallow to price a rare neighbour; **Rank by** switches between evidence and concentration
- **Word Cloud**: a frequency **or keyness** visualisation over any of nine scopes (document, volume, subseries, corpus, collection, user tag, saved search, **custom volume scope**, and a **date range**) shown as a packed spiral cloud (density-aware rotation) or an accessible ranked/weighted list; a **Size words by** control switches between **Frequency** and **Distinctive** — the latter scoring each word by log-likelihood **keyness** against the bundled 552-volume corpus reference (`keyness-baseline.json`), reporting both the log-likelihood score and the effect size as a plain multiple (they disagree, because G² grows with corpus size), listing only words used more here than corpus-wide, disclosing on screen how many of the scope's words were eligible and the corpus frequency below which a word is unpriced, and refusing to compare at all when live tokenisation settings diverge from the reference (naming the setting) or when the lens has no baseline (the three entity lenses); a date-range cloud is reached from Chronology's **"Word Cloud for this range"** action and offers a **"View in Chronology"** item to hand back to the same range (macOS also exposes a "Date Range" scope entry with inline start/end date pickers); device-local **Appearance** controls in Settings set the cloud's font (Rounded/Default/Serif/Monospaced) and density (Compact/Balanced/Airy), applied across the cloud, comparison columns, and image exports; **semantic lenses** narrow the cloud to People / Places / Organizations (on-device NaturalLanguage named-entity recognition), Topics / Actions / Descriptors (part-of-speech), or **Concepts / Sentiment** (bundled lexicons; sentiment is colour-coded by polarity); **tap a word to chart its frequency across the whole corpus in Corpus Analytics** (volume-, subseries-, and custom-scope clouds add an optional "analyze within this volume/subseries/scope" menu action, and "Search for this term" stays on the word's context menu); compare two scopes side by side; **export** the cloud from its Options menu as an image (PNG), a PDF, or a ranked CSV (Rank / Term / Occurrences / Share) carrying the same `#`-commented provenance preamble — including how many words you hid by hand, and no dating rule or year range, since a cloud never reads a document date — named `FRUS-WordCloud-…`; hide a word **in all clouds, just the current lens, or just the current cloud** (the per-cloud hide is temporary — the word returns next time the cloud is generated — and "Show N hidden words" restores all of them), and a **Word Cloud settings pane** with custom global + per-lens stop lists and tunable criteria (minimum length/occurrences, plural folding, classification-marking and diplomatic-boilerplate filters); corpus/subseries results are precomputed in the background on iOS and cached on disk
- **Working Corpora** (M-1): named, fixed **document**-grain scopes — the analogue of a volume scope one level down. Capture a result set with **Save as Working Corpus** on either platform, apply it from the search filters, manage it in Settings → Working Corpora. The **key list syncs**, not the query that produced it, so "the 267-document corpus" is 267 documents on every device and counts taken inside it reproduce; each surface states its indexed coverage ("142 of 267") rather than silently resolving to a smaller set. Composes with a project History/Focus scope by **intersection**, and a disjoint pair matches nothing rather than degrading to a whole-corpus search
- **Custom Volume Scopes**: named, reusable volume sets synced via iCloud, managed in Settings → Research → Volume Scopes on both platforms — create, edit, and delete scopes; the editor adds volumes by name search or through an **"Add Volumes By…"** facet menu (Subject, Person, Manifest Tag, Coverage Years / Editor — facet matches union into the selection, never removing volumes already picked), and each scope row offers a one-tap word cloud. Apply a scope anywhere volumes are scoped: the Search filters' **"My Volume Scopes"** section (applying seeds the volume picker with the scope's *indexed* members, showing "N of M volumes indexed" and warning when none are indexed yet), the Corpus / Person / Cross-Reference Analytics scope menus (zero-indexed scopes disabled), the Word Cloud scope menu, and the About the Series dashboards at manifest grain
...
- **Zotero** — a single unified **Send to Zotero…** action: with a connected account (Settings → Connections → Zotero, Web API key; the connection syncs across devices via iCloud Keychain) it pushes a single document — or a whole collection — into your Zotero library over the Web API, carrying tags and research notes; without an account it falls back to an RIS file for desktop import (File → Import). The former split between a "Zotero RIS" export format and a separate "Send to Zotero Library" button is collapsed into this one menu
- CloudKit-synced user data (notes, tags, collections, projects, highlights) with live sync monitoring and proactive health checks: account status and private zone verification at launch and on foreground; macOS status bar surfaces zone-missing and not-signed-in warnings alongside the existing "Syncing…/Synced/Sync Error" states; iOS Settings shows the same diagnostics in the iCloud Sync section; a **Sync Log** (Settings → Data & Recovery → Diagnostics on both platforms) keeps a local, on-device, **redacted** log of iCloud sync events — event types, timing, and error codes only, never record/account identifiers or any synced content — that you can read and export to help diagnose sync problems
...
- Settings → Volumes & Storage: per-category usage breakdown (volume XML / search index / AI summaries / total); indexing controls (Index Remaining, Reindex All, Delete & Rebuild) positioned above the volume list; per-volume reindex and remove controls; storage limit with pre-download gate and Manage Storage sheet showing LRU removal candidates
...
| **StorageReindexTest** | Open Settings → Volumes & Storage; confirm indexing controls appear above the volume list; confirm per-volume indexed badge; tap Reindex on an indexed volume; confirm it re-indexes and badge updates; confirm per-category size breakdown (volumes / index / summaries / total) is displayed |
| **DeleteIndexRebuildTest** | Open Settings → Volumes & Storage; tap "Rebuild From Scratch"; confirm confirmation alert appears; confirm that after accepting, all volumes are re-indexed from scratch and search results are correct |
...
| **macOSSettingsSceneTest** | Open macOS Settings via ⌘,; verify all panes open correctly; in the Notes pane, confirm rows have consistent horizontal padding; in Volumes & Storage, confirm per-category breakdown appears and indexing controls are above the volume list |
...
| `frus.history` | History | — | Complete reading + search history: project scope, free-text filter, per-entry delete (hosts the shared `HistoryView`; also reachable via the History menu's "Complete History…" item) |
